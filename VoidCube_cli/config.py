...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade VoidCube-agent"
...
# Re-export from VoidCube_core.constants — canonical definition lives there.
from VoidCube_core.constants import get_VoidCube_home, get_config_path, get_env_path  # noqa: F811,E402
...
DEFAULT_CONFIG = {
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "> *Voidcube Agent*" header.
...
def _provider_key_from_name(name: str, fallback: str = "provider") -> str:
    """Build a stable provider key from a display/provider name."""
...
def show_config():
    ...
    active_provider_cfg = providers_cfg.get(active_provider, {}) if active_provider in providers_cfg else {}
    ...
    if terminal.get('backend') == 'docker':
        ...
        print(f"  Podman image: {terminal.get('podman_image', 'nikolaik/python-nodejs:python3.11-nodejs20')}")
...
def set_config_value(key: str, value: str):
    ...
    api_keys = [
        'OPENROUTER_API_KEY', 'OPENAI_API_KEY', 'VOICE_TOOLS_OPENAI_KEY',
