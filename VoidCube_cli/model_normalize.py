...
Different OpenAI-compatible providers expect model identifiers in different formats:

- **Aggregators** (OpenRouter, Nous, AI Gateway, Kilo Code) use
  ``vendor/model`` slugs such as ``openai/gpt-5.4``.
- **Copilot** expects bare model names.
...
#
# Example: "gpt-5.4" -> first token "gpt" -> vendor "openai"
#          -> aggregator slug: "openai/gpt-5.4"
...
def _normalize_for_deepseek(model_name: str) -> str:
    ...
    - Contains any reasoner keyword (r1, think, reasoning, cot, reasoner, pro)
      -> ``deepseek-v4-pro``.
    ...
    Returns:
        One of ``"deepseek-v4-flash"`` or ``"deepseek-v4-pro"``.
    """
...
def _strip_vendor_prefix(model_name: str) -> str:
    ...
    Examples::
        ...
        >>> _strip_vendor_prefix("openai/gpt-5.4")
        'gpt-5.4'
        >>> _strip_vendor_prefix("gpt-5.4")
        'gpt-5.4'
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to Voidcube' canonical ids."""
    raw = (provider_name or "").strip().lower()
    if not raw:
        return raw
    try:
        from VoidCube_cli.models import normalize_provider
...
def detect_vendor(model_name: str) -> Optional[str]:
    ...
    Returns:
        The vendor slug (e.g. ``"openai"``) or ``None``
...
def _prepend_vendor(model_name: str) -> str:
    ...
    Examples::
        ...
        >>> _prepend_vendor("gpt-5.4")
        'openai/gpt-5.4'
        >>> _prepend_vendor("openai/gpt-5.4")
        'openai/gpt-5.4'
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        model_input: The bare or vendor-prefixed model name.
        target_provider: The canonical Voidcube provider id, e.g.
            ``"openrouter"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``VoidCube_cli.models.normalize_provider()``.
    ...
    Examples::
        ...
        >>> normalize_model_for_provider("glm-5", "zai")
        'glm-5'
    """
