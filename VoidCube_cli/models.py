...
Add, remove, or reorder entries here — both `/api` and
`VoidCube` provider-selection will pick up the change automatically.
"""
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from VoidCube_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def fetch_openrouter_models(
    ...
    *,
    ...
    """Return the curated OpenRouter picker list with all free models included."""
    ...
    for item in live_items:
        ...
        if _openrouter_model_is_free(item.get("pricing")):
            free_models.append((mid, "free"))

    # Build curated list from preferred models
    ...
    # Add free models that are not already in curated list
    curated_ids = {mid for mid, _ in curated}
    
    # Deduplicate free models to avoid duplicates like ":free:free"
    seen_free_models = set()
    for free_model, desc in free_models:
        # Skip if already in curated or already added
        if free_model not in curated_ids and free_model not in seen_free_models:
            seen_free_models.add(free_model)
            # Also check for potential duplicates (e.g., nvidia/nemotron-3-super-120b-a12b vs nvidia/nemotron-3-super-super-120b-a12b)
            is_duplicate = False
            for existing in curated_ids | seen_free_models:
                # Check if this is a duplicate with extra suffix
                if (free_model.startswith(existing) or existing.startswith(free_model)) and free_model != existing:
                    # Prefer the shorter/cleaner name
                    if len(free_model) > len(existing):
                        is_duplicate = True
                        break
            if not is_duplicate:
                curated.append((free_model, desc))

    # Sort: recommended first, then free models, then others
    first_id, _ = curated[0]
    curated[0] = (first_id, "recommended")
    
    # Sort the rest: free models first, then others alphabetically
    rest = curated[1:]
    rest.sort(key=lambda x: (0 if x[1] == "free" else 1, x[0]))
    curated = [curated[0]] + rest
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in _PROVIDER_ORDER:
        ...
        has_creds = False
        try:
            from VoidCube_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def detect_provider_for_model(
    ...
    # If someone types `/model nous` or `/model deepseek`, treat it as a
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``VoidCube_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Voidcube should expose the /fast toggle for this model."""
...
def _provider_model_ids_unfiltered(
    ...
    *,
    ...
    if normalized == "nous":
        ...
        try:
            from VoidCube_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from VoidCube_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "VoidcubeAgent/1.0",
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from VoidCube_core.constants import AI_GATEWAY_BASE_URL
