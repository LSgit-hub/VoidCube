...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("provider", "快速配置API Provider",
               "配置管理", args_hint="[name] [api_key]", aliases=("pk",)),
...
def telegram_bot_commands() -> list[tuple[str, str]]:
    ...
    for cmd in COMMAND_REGISTRY:
        ...
        if tg_name:
            result.append((tg_name, cmd.get_description()))
    return result
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        for cmd_name in sorted(plugin_cmds):
            ...
            try:
                from VoidCube_cli.i18n import t
                desc = t('commands.plugin_command', default='插件命令')
            except Exception:
                pass
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``VoidCube skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /VoidCube handler.

    Maps both canonical names and aliases so /VoidCube bg do stuff works
    the same as /VoidCube background do stuff.
    """
    overrides = _resolve_config_gates()
