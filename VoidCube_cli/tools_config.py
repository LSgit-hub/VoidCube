"""
Unified tool configuration for Voidcube Agent.

`VoidCube tools` and `/api tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.VoidCube/config.yaml under
...
# 简化的 nous 订阅功能检查
def get_nous_subscription_features(config):
    """简化的订阅功能检查 - 返回空功能列表"""
    class MockFeatures:
        features = {}
    return MockFeatures()
...
from VoidCube_cli.cli_output import (  # noqa: E402 — late import block
...
from VoidCube_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_VoidCube_home()}/VoidCube-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in VoidCube-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "VoidCube-cli" (which include all _VOIDCUBE_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "VoidCube-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        ...
        # Also include the original composite toolset names directly so
        # downstream get_tool_definitions can resolve the full tool hierarchy
        # (including skills, ops sub-toolsets, etc.) rather than only the
        # leaf configurable toolsets above.
        from tools.toolsets import is_valid_toolset
        for ts_name in toolset_names:
            if is_valid_toolset(ts_name):
                enabled_toolsets.add(ts_name)

    # Plugin toolsets: enabled by default unless explicitly disabled.
    # A plugin toolset is "known" for a platform once `VoidCube tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by VoidCube tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (VoidCube-cli, VoidCube-telegram, etc.)
...
def _prompt_toolset_checklist(platform_label: str, enabled: Set[str]) -> Set[str]:
    ...
    from VoidCube_cli.curses_ui import curses_checklist
    from tools.toolsets import resolve_toolset
    ...
    if not env_vars:
        ...
        if managed_feature:
            ...
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.VoidCube/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `VoidCube tools` and `/api tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("> Voidcube Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://VoidCube-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from VoidCube_core.constants import display_VoidCube_home
    print(color(f"  Tool configuration saved to {display_VoidCube_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'VoidCube' or gateway restart.", Colors.DIM))
    print()
