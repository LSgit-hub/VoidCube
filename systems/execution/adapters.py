...
class SelfLearningExecutionAdapter:
    ...
    async def execute_self_learning_followup(self, request: dict | None = None) -> Dict[str, Any]:
        ...
        task_id = str(task.get("task_id") or request.get("task_id") or "")
        trace_id = str(task.get("trace_id") or request.get("trace_id") or "")
        title = str(task.get("title") or request.get("title") or "Self-learning follow-up").strip()
        summary = str(task.get("summary") or request.get("summary") or "").strip()
        if not title:
            title = "Self-learning follow-up"
        ...
        profile = derive_runtime_task_profile(
            ...
            task_family=task.get("task_family") or metadata.get("task_family"),
            execution_kind=(
        ...
        if profile.get("governance_task_type") != "self_learning":
            raise HTTPException(
                status_code=400,
        ...
        skill_observations = [
            str(observation)
            for observation in skill_evidence.get("observations") or []
        ]
        topic = self.learning_service.create_topic(
            title=title,
            reason=summary or "Supervisor approved a learn-only follow-up task.",
            source="self_learning",
            tags=["supervisor-followup"],
            metadata={
                "task_id": task_id,
                "trace_id": trace_id,
                "source_task_metadata": metadata,
                "skill": dict(skill_execution.get("skill") or {}),
                "skill_delegate": skill_execution.get("delegate"),
            },
        )
        session = self.learning_service.plan_session(
            topic=topic,
            planned_minutes=int(constraints.get("planned_minutes", 20)),
            trigger="idle",
            notes=[
                "Executed by the canonical executor self-learning adapter.",
                "Delegated learning guidance to the bundled self-learning skill contract.",
                "No body upgrade, body switch, or memory mutation was performed.",
            ],
        )
        ...
        observations.append("Supervisor approval was treated as evidence for a learn-only follow-up.")
        experiment = self.learning_service.record_experiment(
            topic=topic,
            session=session,
            hypothesis=str(
                evidence.get("hypothesis")
                or "A supervised learning follow-up can preserve useful context without mutating runtime state."
            ),
            method=str(
                evidence.get("method")
                or "Apply the bundled self-learning skill guidance, then create an auditable learning topic, session, experiment, and conclusion from the approved task."
            ),
            observations=observations,
            outcome=str(evidence.get("outcome") or "inconclusive"),
            compared_against=list(
                evidence.get("compared_against")
                or skill_evidence.get("comparisons")
                or ["direct runtime mutation"]
            ),
        )
        recommendations = [
            LearningRecommendation.model_validate(item)
        ...
        conclusion = self.learning_service.submit_conclusion(
            ...
            comparisons=list(evidence.get("comparisons") or ["learn-only execution > direct mutation"]),
            summary=(
                str(evidence.get("conclusion_summary") or "")
                or f"Recorded self-learning skill-guided follow-up for supervisor task '{title}'."
            ),
            verified=bool(evidence.get("verified", False)),
            recommendations=recommendations,
        )
        supervisor_submission = self.learning_service.build_supervisor_payload(conclusion)
        supervisor_submission.setdefault("metadata", {})
        supervisor_submission["metadata"].update(
            {
                "trace_id": trace_id,
                "source_task_id": task_id,
                "source_governance_task_type": profile["governance_task_type"],
                "source_task_family": profile["task_family"],
                "skill_delegate": skill_execution.get("delegate"),
                "skill_backend": skill_execution.get("backend"),
        ...
        return self.attach_execution_route_hint(
            {
                ...
                "supervisor_submission": supervisor_submission,
            },
