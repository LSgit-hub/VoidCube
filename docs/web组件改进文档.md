...
首页只保留一份场景摘要和一份连接状态。底部 Dock 保留替身快速状态条，用于显示当前角色、健康度和迭代等级；Auto 的“闭环总览、判断依据、当前判断、观察记录”统一为一个“自主观察”入口，在面板内用标签切换。花盆、电脑、纸稿和时钟仍是四个业务域的主入口，交付、健康、提升审计、运行配置和账号保留为互不重叠的工具入口。

旧的 `lminput`、`cognition`、`observation` panel key 只作为内部标签和迁移兼容路由保留，不再作为首页独立 Dock 按钮或独立信息架构入口。
