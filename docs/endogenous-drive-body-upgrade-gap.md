...
此路径**不由内生驱动触发，也不由自主链路 backlog / 分段投影直接驱动**。必须手动调用 API。
...
**架构约束（§3.8 / §7.5）**：身体切换不由自主链路 backlog 直接驱动。Governor 保有否决权，但健康值/Governor 批准都只是“建议切换”的程序前置门；真正 `activate_slot` 需用户同意（目标语义，待实现）。
...
| # | 缺失环节 | 描述 |
|---|---------|------|
| **1** | 学习→改进任务类型 | 当前自主链路 backlog / 分段读模型里缺少稳定的 `body_improvement` 闭环投影。创造力候选更偏"只读研究"，而不是"基于研究编辑代码" |
