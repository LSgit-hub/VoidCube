...
- Web 监控只作为 API-B 观测面，展示监督者动作、反馈、任务治理、记忆状态和自主任务回报；它不是用户聊天入口，也不监控、接管或手动控制用户链路 / 自主链路。
...
- `systems/supervisor/ui_runtime.py`：Web 小屋，只看 API-B 状态、反馈、任务治理、记忆状态和替身信息，不承担聊天入口或执行控制台职责
...
- `gateway/`：内部网关和服务注册
- `memory/`：服务化记忆层
- `supervisor/`：监督者、任务治理、内生驱动、UI/runtime；目标语义为全天候治理，当前 `/auto` 开关仅是临时启停门控
...
- `gateway/internal_gateway.py` 管的是 agent scene 两条执行位：`user_chat` 表示用户交互，`supervisor_task` 表示监督者放行的学习/改造任务；两者都属于 API-A 执行侧。
