...
- VoidCube 的运行必须区分两条链路：用户主 CLI → API-A 主 Agent 的“用户链路”，以及 API-B 内生驱动 → 任务列表 → API-A 自主执行面 → Mem → API-B 再读取的“自主链路”。两者共享 API-A 能力域和 Mem，但交互语义互不覆盖。
- 监督者目标语义是全天候运行；`/auto` 开关只是当前仍保留的自主链路临时启停门控，不再限制主 CLI 输入，也不阻断用户与主 Agent 交互。
- 自主任务采用 pull 形态：监督者管理任务列表，API-A 自主执行面（当前表现为由 `/auto` 临时门控的最小迷你 CLI / 子代理通道）通过自主任务执行通道拉取 `self_learning` / `body_improvement`。
