...
- 监督者目标语义是全天候运行；`/auto` 开关只是当前仍保留的自主链路临时启停门控，不再限制主 CLI 输入，也不阻断用户与主 Agent 交互。
- 自主任务采用 pull 形态：监督者管理治理在途链路项，API-A 自主执行面（当前表现为由 `/auto` 临时门控的最小迷你 CLI / 子代理通道）通过自主任务执行通道拉取 `self_learning` / `body_improvement`。
...
- 服务启停、状态面板和一些本地运维命令
- API-A 自主执行面的轻量辅助逻辑；主 CLI 只保留 `/auto` 临时门控和用户链路交互

这里要特别分清三个高可见入口：

- `VoidCube_cli/status.py`：主 CLI 状态面，只读 `user_chat`，服务用户链路，不展开自主执行细节
- 主 CLI 内嵌最小 CLI 组件：`/auto` 启动同一 CLI 内的自主链路专用跟踪展示组件，先展示 API-B 模型生成 / 候选 / 判断状态，再在 API-B 转交后由同一组件内的 API-A 自主执行面负责 poll / 认领 / 执行 / 回写 `supervisor_task` 自主链路任务；完全空态隐藏，不影响用户主输入
...
- `gateway/`：内部网关和服务注册
- `memory/`：服务化记忆层
- `supervisor/`：监督者、API-B 判断、内生驱动、UI/runtime；目标语义为全天候判断与记忆管理，当前 `/auto` 开关仅是临时启停门控
...
- `/auto` 不再代表“用户不可输入的独占模式”，而只是当前自主链路的临时启停入口
