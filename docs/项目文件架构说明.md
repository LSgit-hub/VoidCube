...
│  ├─ autonomous_runner.py         # API-A 自主执行最小 CLI；认领/执行/回写自主链路任务
...
- `VoidCube_cli/autonomous_runner.py`：`VoidCube autonomous` / `VoidCube auto-cli` 专用 API-A 自主执行最小 CLI，负责 poll / 认领 / 执行 / 回写自主链路任务
