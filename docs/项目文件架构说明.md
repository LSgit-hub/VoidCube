...
│  ├─ ops/
│  │  ├─ serve.py                  # 服务启停
│  │  ├─ dashboard.py              # 最小 CLI 观察窗；专看自主链路 API-A 执行位
│  │  ├─ executor.py               # 执行器入口
│  │  └─ provider.py
│  ├─ tools/                       # CLI 自带工具包装
│  └─ locales/
...
- `VoidCube_cli/ops/dashboard.py`：最小 CLI 观察窗，只读 `supervisor_task` 与 `active_cli_executor(agent_lane=supervisor_task)`，服务自主链路 API-A 执行观察
