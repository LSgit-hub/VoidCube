...
当前状态：M0、M1、M2 已完成；M3 主实现与聚焦测试已完成，正在执行全量验收门。验收完成前仍不得提前开展 M4 自主治理联调或模块配合探索。
...
2. 收口 Store/Mem 恢复规则、自主任务终态和执行回写，完成 M4 治理与执行闭环。
3. 为 Gateway 注册、服务 lifespan、自主任务终态和身体切换补齐跨服务集成测试，并完成 M5 构建与隔离安装验收。
