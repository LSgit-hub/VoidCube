...
| `VoidCube_cli/` | 参数、配置、命令、UI、认证和本地生命周期 | 长期记忆领域规则 |
| `cli.py` | 交互式会话和展示桥接 | 新增服务端治理逻辑 |
...
- `response_disposition.py`：兼容内容、工具调用和文本/空响应 disposition 判定；
...
| M1 用户链路收口 | 先保证直接服务用户的链路可维护 | 收口请求/响应、传输、单轮/attempt 状态、retry/compression recovery、response disposition 判定和 finalization 边界，从 `run_agent.py` 提取 disposition action 应用与循环时序；再拆 `cli.py` 会话 UI；同轮删除原实现 | 流式/非流式、工具调用、上下文恢复、模型切换、持久化和中断测试全部通过；主 CLI 行为不变 |
