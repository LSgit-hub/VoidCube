...
- 混合评分：词项覆盖率 + 新近度 + Tier 1 relevance 或 Tier 2 importance/dynamic weight；
...
Memory health 暴露召回请求数、命中数、失败数、命中率、最近结果数和耗时，避免“服务健康”掩盖长期无命中的空转状态。
...
- 当前召回是可解释的多语言混合词项检索，不宣称向量语义；正式 embedding 仍需定义模型/版本、回填、失效和迁移协议；
