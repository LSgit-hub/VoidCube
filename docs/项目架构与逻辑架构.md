...
- `context_engine.py` / `context_compressor.py`：上下文预算、压缩和恢复；
...
| M1 用户链路收口 | 先保证直接服务用户的链路可维护 | 收口请求/响应、传输、单轮/attempt 状态和 retry execution 边界，从 `run_agent.py` 提取 compression/context recovery 执行与 turn finalization；再拆 `cli.py` 会话 UI；同轮删除原实现 | 流式/非流式、工具调用、上下文恢复、模型切换、持久化和中断测试全部通过；主 CLI 行为不变 |
