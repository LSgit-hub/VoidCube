...
完成轮次进入 Tier 1 的同一事务会运行确定性 Profile 白名单提取，只接受用户第一人称明确声明的稳定称呼、语言、工具偏好、回答风格和有限事实；助手推断、含不确定或临时标记的表达、未分类自由文本与敏感值不得写入 Profile。即时提取与批量压缩共用 `profile_store.py` 的 upsert/supersede 规则。明确撤销按 owner/workspace 删除当前 Profile、embedding、派生 Tier 2 与召回引用，并写入 `profile_memory_tombstones`；tombstone 保存撤销时间和证据 turn，只允许撤销后的新声明建立新值，阻止旧 turn 在后续压缩中复活。
