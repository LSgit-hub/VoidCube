...
| `VoidCube_cli/` | 参数、配置、命令、UI 状态、认证和本地生命周期；`chat_render_state.py` 统一持有流式/推理展示状态 | 长期记忆领域规则 |
| `cli.py` | 交互式会话、渲染协调和命令桥接 | 平铺复制流式状态；新增服务端治理逻辑 |
...
| M1 用户链路收口 | 先保证直接服务用户的链路可维护 | Agent 侧请求/响应、传输、状态、恢复、disposition、工具 turn 与 finalization 各有唯一所有者；CLI 流式状态已归 `chat_render_state.py`，继续提取纯渲染转换和命令路由；同轮删除原实现 | 流式/非流式、工具调用、上下文恢复、模型切换、持久化和中断测试全部通过；主 CLI 行为不变 |
