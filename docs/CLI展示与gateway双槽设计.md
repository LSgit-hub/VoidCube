...
> **2026-07 对齐说明**：本文只讨论可观测性分槽。Supervisor 启动后自主链路门控默认关闭；`/auto` 临时启用 Supervisor 周期任务和主 CLI 内的 API-A 自主执行组件，`/auto-q` 临时停用。判断 `supervisor_task` lane 时以“当前正在执行监督者任务”为准，不能把整个主 CLI 会话理解成被 `/auto` 接管。Web 小屋只作为 API-B 观测面，不成为用户聊天入口。
...
| 嵌入式最小 CLI 组件（主 CLI 内的 `auto_execution_panel` + 自主执行 host） | 自主链路中 API-B 状态跟踪与 API-A 子代理执行学习 / 改造任务的专用展示执行面 | 显示 API-B 模型生成 / 候选 / 判断状态；poll / 认领 / 执行 / 回写 `supervisor_task` 链路项；只在 `/auto` 启用且存在 API-B 活动或 API-A 执行内容时显示，不掺入用户交互子代理 |
...
补充边界：主 CLI 不直接用用户交互 host 去 poll、认领或注入自主任务 prompt。`/auto` 启用同一 CLI 内的嵌入式自主执行组件；API-A 自主执行流程只出现在该组件的按需面板里，空态不显示，不抢主输入区。

### 3.1 使用方式

- 用户链路：运行 `VoidCube` 或 `VoidCube chat`，主 CLI 只服务用户输入和 API-A 主 Agent 输出。
- 自主链路：执行 `/auto` 后，Supervisor 启动 API-B 内生驱动与治理复核，主 CLI 同时启动嵌入式 API-A 自主执行组件。组件独立展示 API-B 模型生成 / 候选 / 判断状态，并承载 `supervisor_task` 的 poll、认领、执行和回写；有 API-B 活动或 API-A 链路项执行时显示，完全空闲时隐藏。
