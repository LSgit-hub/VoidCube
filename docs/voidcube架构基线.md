...
**一个以 CLI 为用户入口、以内部网关为神经中枢、以 Mem 为长期记忆与治理灵魂、以监督者为内生认知核心与治理输出中枢及自主链路治理存储管理者、以 Agent 为学习任务与身体升级执行体、以执行器为身体切换机械执行面、以可替换子 Agent 为主要升级对象的单机多进程母体系统。**
...
VoidCube 当前必须把两条执行链路作为一等架构边界，而不是把它们都解释成 CLI 的不同模式：

| 链路 | 简称 | 驱动源 | 执行面 | 观测面 | 记忆闭环 | 边界 |
| --- | --- | --- | --- | --- | --- | --- |
| API-B 内生驱动 → 判断在途存储 → API-A 自主执行体 → Mem → API-B 再读取 | **自主链路** | API-B（Mem / Supervisor）内生感知、判断、生成判断投影与任务 | API-A 域中的自主任务执行面；Supervisor 默认常驻运行，主 CLI 的 `/auto` 只接入本地最小迷你 CLI / 子代理观测执行面，供 API-A 子代理执行 `self_learning` 与 `body_improvement` | Web 监控只展示 API-B 的状态、反馈、判断事项、内生驱动与由 API-A 回报给 API-B 的自主任务状态 | 学习成果、替身改进报告、任务 outcome 必须写入 Mem，并由下一轮 API-B 内生驱动读取和管理 | 不能接管用户主 CLI，不能把 Web 监控变成用户聊天面，不能让 `/auto` 开关成为用户链路模式 |
...
`/auto` 与 `/auto-q` 只是主 CLI 内本地自主链路观测/执行面的接入与退出命令。它们不是 Supervisor 自主链路主启停开关，不是主 CLI 的另一种用户交互模式，也不是用户链路是否可用的开关。自主链路由 API-B 节律和治理条件随 Supervisor 常驻运行，不需要用户手动打开 `/auto` 才启动。

“最小迷你 CLI”只属于自主链路执行面：它服务 API-A 子代理执行学习和替身代码编写，并把进度、状态、结果回报给 API-B / Mem 供用户观测。它不是用户主 CLI 的替代入口，也不应承担用户聊天、主 Agent 展示或 Web 监控职责。

Web 监控只属于 API-B 观测面：展示监督者内生驱动、治理在途投影、治理反馈、API-B 记忆/状态以及 API-A 自主任务回报给 API-B 的状态。Web 监控不得成为 API-A 用户聊天入口，也不得展示成“主 CLI 被接管”。
...
- 独占长期规划状态
- 未经监督者裁决自行触发身体切换
- 自行裁决学习任务是否执行（任务由监督者内生驱动产出并放入治理在途存储）
- 执行身体切换的机械流程（由执行器执行）
...
- 当前 `active` Agent 是正在服务用户的子 Agent；自主链路中的学习任务和身体升级由 API-A 域的自主执行面运行，并应与主 CLI 用户交互隔离
...
- **身体槽位**：`active / shell / candidate / probe / awaiting_user_consent / retired`，解决“哪个子 Agent 服务用户、哪个替身被培养与验证、哪个替身正等待最终同意”的问题。
- **gateway agent 双泳道**：`user_chat / supervisor_task`，解决“同一个 API-A 域的用户交互与监督者任务可观测性互不覆盖”的问题。`user_chat` 属于用户链路；`supervisor_task` 属于自主链路。`supervisor_task` 表示任务来源是监督者，执行者仍是 API-A，不是 API-B。
...
- **Tier 2 编年史压缩**（ChroniclePipeline）：将超过保留窗口的 turns 批量送入 ChroniclePipeline，**LLM 优先 + 启发式降级**。LLM 只从 API-B 的 `memory.llm.*` 配置解析：例如 `memory.llm.provider=deepseek` 时读取 `memory.llm.api_key_env=DEEPSEEK_API_KEY` 对应凭据，不再回退到 API-A 或 `OPENAI_API_KEY`。凭据可用时使用 `LLMEventExtractionBackend`（LLM 理解语义提取事件）和 `LLMScholarBackend`（LLM 生成场景/弧线/纪元摘要）；API-B 凭据不可用时才显式降级为 `HeuristicEventExtractionBackend`（关键词正则匹配）和 `HeuristicScholarBackend`（模板填充）。**LLM 压缩才能真正理解内容语义——区分"决定重构架构"和"嗯好的"，而不是仅靠关键词匹配。** 压缩不可逆，但通过 source\_turns 保留反向引用链路。
...
**规则执行机制（双路径冗余）**：五条压缩规则通过两条独立路径执行，确保自主链路启用与否都不影响记忆维护规则生效：
...
学习任务不是内生驱动的本体，而是当前治理输出中的一种创造类兼容投影。当前阶段，这类投影主要表现为探索式研究任务，并作为身体改造进化的证据来源。学习任务不是由独立的"自学系统"服务执行，也不是由用户主 CLI 接管执行，而是由 API-A 域中的自主任务执行面直接执行（当前实现仍由 `/auto` 临时开关启停）。学习之后，如果监督者判断证据积累达到阈值，还会继续产出 `body_improvement` 任务，由同一 API-A 自主执行面在 shell 槽位执行代码改进。

当前进入治理在途存储的创造类兼容任务投影主要包括：
...
API-A 自主执行面（最小迷你 CLI / 子代理通道；由主 CLI 内 `/auto` 接入本地观测/执行面）
  → 通过网关遍历治理在途存储
...
**关键纠正**：不存在独立的"自学系统"运行服务。学习任务的生产者是监督者（API-B），执行者是 API-A 自主执行面，学习成果的存储位置是 Mem。当前代码中 `systems/self_learning/` 只保留历史学习结论 contract / store：学习任务的生产归监督者，执行归 API-A 自主执行面，存储归 Mem；用户主 CLI 只属于用户链路，不承担自主任务执行槽。
...
- 直接启停 Agent 进程
- 直接执行蓝绿切换（由执行器执行）
- 直接执行学习任务（由 API-A 自主执行面通过自主任务执行通道执行）
- 直接编辑替身代码（由 API-A 自主执行面根据学习成果执行身体升级）
- 直接向 Agent 推送任务（Agent 主动遍历治理在途存储）
- 承担长期机械执行器角色
...
- **当前实现**：治理在途存储治理当前主要由 `evaluate_endogenous_drive()` 输出的 `drive_input`、`review_autonomous_chain_tasks()`、`decide_autonomous_chain_task()` 和 `AutonomousChainStore.update_status()` 的规则、阈值与合法状态迁移控制；`activity_guards()`/`activity_guards` 仅保留为兼容探针入口，不再是前台或新执行链路的正式主协议
...
```text
监督者内生驱动（治理输出中的创造类兼容投影）→ 产出学习任务 → 放入治理在途存储
  → API-A 自主执行面执行学习任务 → 学习成果写入 Mem
    → API-A 自主执行面根据成果编辑 shell 替身代码（身体升级）→ 进展写入 Mem
      → 监督者整理记忆 → 内生驱动判断替身进展
...
监督者的目标语义是全天候运行：内生驱动器按配置周期（默认 300s）运行认知评估与治理输出，其中只有适合投影为 API-A 自主执行面执行的创造类输出，才会进入治理在途存储，当前主要是探索式学习任务（`self_learning`）以及达到阈值后产生的替身改进任务（`body_improvement`）。

**当前实现状态**：Supervisor 默认以 `autonomous_chain_start_on_boot=True` 随服务启动自主链路 review / endogenous-drive 循环；`/auto` 与 `/auto-q` 只负责主 CLI 内本地观测/执行面的接入与退出。普通 deactivate 在常驻模式下会跳过停机，只有显式 `force` 用于紧急或诊断停机。`/auto` 不限制主 CLI 的用户输入，不会让 Gateway 用户请求返回 503，也不阻断用户使用主 Agent。

这里还要强调一点：**监督者的关键不只是“自动产任务”，而是“接管治理在途存储管理责任”。** 如果只有产出而缺少治理，那么系统仍然只是程序循环，不是最初设计里的“监督者身份”。

正式身体切换不由自主链路治理在途存储驱动，而是由监督者在整理记忆后内生判断替身进展情况，形成"建议切换"裁决。

**身体切换的用户同意门**：替身的代码改进与 probe 验证仍由监督者 / API-A 自主执行面全天候自主进行（改的是替身槽代码，不影响用户使用），但**真正激活新替身（activate_slot）这最后一步只能由用户同意后执行**。监督者/Governor 的裁决产出"建议切换"，系统据此置 `awaiting_user_consent`；用户经 `/body/switch/consent` 显式同意后才由执行器激活。Governor 的独立审查（否决权）作为同意之前的程序前置门，与用户同意门叠加，而非替代。
> 当前代码事实：`execute_body_upgrade` 在 probe 通过后提交 `switch_request`，Governor 只产出 `awaiting_user_consent` 和 `await_user_consent` 动作；不会自动 `activate_slot`。用户同意后才通过 `probe_to_active` 审查触发 `activate_slot` 与 watch window。
...
- 执行 probe（结构化健康检查）
- 启停目标 Agent 进程
- 推进状态迁移（shell → candidate → probe → awaiting_user_consent → active → retired）
...
- 解释长期记忆（Mem / 监督者负责）
- 替代监督者做治理裁决
- 执行学习任务（API-A 自主执行面负责）
- 编辑替身代码 / 身体升级（API-A 自主执行面负责）
...
治理在途存储（即监督者管理的自主链路持久化链路存储）不是独立的架构组件，而是由监督者直接管理的持久化链路存储。监督者会把治理输出中适合交由 API-A 自主执行面执行的兼容任务投影放入此存储，API-A 自主执行面通过网关主动遍历此存储并执行。当前实现的 `/auto` 开关只临时决定自主链路是否启用，不改变该存储的架构归属。

治理在途存储的定位：

- **持有者**：监督者（属于 `systems/supervisor/` 的一部分，对应 `AutonomousChainStore`）
- **生产者**：监督者治理输出中的兼容任务投影层（由内生驱动认知判断后落入治理在途存储）
- **消费者**：API-A 自主执行面（最小迷你 CLI / 子代理通道，通过自主任务执行通道遍历并执行；主 CLI 内 `/auto` 只负责接入本地观测/执行面）
...
治理在途存储不是独立服务，不需要独立进程、独立端口或独立注册。它是监督者模块内部的数据结构，通过网关暴露给 Agent 访问。

当前代码中 `systems/supervisor/autonomous_chain_store.py` 的 `AutonomousChainStore` 已经是治理在途存储的正式实现骨架，现阶段仍需继续完善的是：
...
Agent 可以持有短期工作态，但长期身份、长期记忆、治理历史、演化谱系、治理在途存储和裁决依据都必须落到 Mem 或明确的持久任务存储中。

### 4.5 学习与执行分离

学习任务由监督者内生驱动产出，由 API-A 自主执行面通过自主任务执行通道执行。学习之后触发的 `body_improvement` 任务同样由 API-A 自主执行面执行；身体切换则由执行器执行。

三个动作分属三个角色：

- **监督者**：产出自主任务，管理治理在途存储，判断身体切换时机
- **API-A 自主执行面**：执行学习任务与 `body_improvement` 任务，编辑替身代码（身体升级），提交进展到 Mem
- **执行器**：只执行身体切换机械流程

不存在独立的"自学系统"运行服务。交由 API-A 自主执行面执行的自主任务，其生产者是监督者，执行者是 API-A 自主执行面，成果与进展的存储位置是 Mem。
...
- **当前本地接入：`/auto` 命令**：Supervisor 默认随服务启动自主链路；`/auto` 与 `/auto-q` 只作为主 CLI 内本地观测/执行面的接入与退出命令。该命令不限制主 CLI 输入，不影响用户与主 Agent 交互。
...
监督者管理的治理在途存储包含交由 API-A 自主执行面执行的自主任务，至少包括探索式学习任务（`self_learning`）和替身改进任务（`body_improvement`）。身体切换不由自主链路治理在途存储驱动。
...
- `running → completed`：API-A 自主执行面执行成功，学习成果或改进进展写入 Mem
- `running → failed`：API-A 自主执行面执行失败，或监督者对长时间卡住的任务做正式失败清理
...
- 运行时可以继续使用 `scheduled_for`、顺延、冲突索引等内部治理机制；
- 但 Web 小屋主视图只展示“API-B 判断 → API-A 自主执行 → Mem 写回 → API-B 再读取”的闭环状态；
- 治理在途存储中的其他任务只作为“链路回流 / 只读观测”展示，不再作为用户可操作的旧队列卡。
...
```text
用户 -> 主 CLI -> API-A 主 Agent (`user_chat`) -> 工具 / 必要记忆访问 -> 主 CLI
```

主 CLI 是用户入口。实际用户任务执行由 API-A 主 Agent 完成。长期记忆访问通过受控记忆入口进入 Mem；该链路不依赖自主链路治理存储。
...
API-A 自主执行面主动拉取（pull），监督者不推送（push）。治理在途存储中的主执行对象包括 `self_learning` 和后续的 `body_improvement`；其中学习任务可自主决定使用子代理辅助复杂任务。不存在独立的"自学系统"服务。
...
**关键原则**：健康值达标是"建议"而非"自动"。Governor 保有否决权（程序前置门）；真正激活新替身（activate_slot）需**用户同意**。身体切换不由自主链路治理在途存储驱动。执行器只做切换的机械流程。
...
1. **页面主体保持为“小屋”而不是工具台**：现有房间、人物、Dock、粒子、scene 切换和轻动画可以继续存在；治理结构升级不应把页面重新做成传统控制台或任务表格站。
2. **主视图围绕自主链路四段闭环展开**：
   | 视图段位 | 名称 | 语义 |
   | --- | --- | --- |
   | **第 1 段** | API-B 判断 | 监督者正在判断、维护记忆、筛选候选或处理非创造类治理动作 |
   | **第 2 段** | API-A 自主执行 | API-A 自主执行面正在执行 `self_learning` / `body_improvement` |
   | **第 3 段** | Mem 写回 | 最近完成的自主任务结果、学习结论、替身改进结果正在回流 |
   | **第 4 段** | API-B 再读取 | 监督者基于最新写回与候选继续进行下一轮判断 |
3. **用户链路只作为软感知展示**：Web 小屋可以显示 `active_sessions`、`user_chain_signal.is_quiet` 这类软信号，但必须明确它们只是 API-B 的判断输入，不是聊天监控，也不是用户链路内容面板。
4. **任务与闭环分离**：
   - 闭环卡组展示“当前自主链路走到哪一段”；
   - 任务卡只展示“当前已在链路中的任务位置与状态”；
   - 时间线只展示“刚刚发生了什么”。
   三者不能混成旧式“任务总览 + 队列操作台”。
   - Web 小屋前端应优先消费专门的自主链路读模型，例如 `autonomous_observation.chain.segments / loop.stage_cards / loop.rail_entries / board.primary_focus` 一类投影，而不是直接把 Supervisor 底层任务存储结构当成页面协议。
5. **链路任务只做只读观测**：
   - Web 小屋可以显示 API-B 当前任务、API-A 当前任务、链路回流任务和候选；
   - 但这些展示的目标是帮助理解“API-B 在做什么”，不是提供一套人工管理队列的操作面。
6. **统计必须同时反映 active / shell 等身体槽位**：不能只显示 `active_slot`。当 active 在 `slot-A` 而 Agent 正在编辑 `slot-B` 时，UI 必须能同时表达这两个槽位，避免把“正在编辑 shell 槽”误显示成“只存在 active 槽”。
7. **场景（scene）按报告者分域**：场景不是全局单值。每个运行实体只声明自己的“当前在做什么”——
   ...
   | <br />                      | <br />   | `planning`     | 决策/批准/拒绝任务（管理治理在途存储） |
   ...
   | <br />                      | <br />   | `executing`    | 正在执行自主链路中的其他执行任务；用户主 CLI 的 `user_chat` 不在 Web 监控中展开 |
   | **执行器 (Executor)**          | 身体切换机械面  | `idle`         | 无活动                |
   | <br />                      | <br />   | `body_switch`  | 正在执行身体切换流程         |
   显式边界：
   - 监督者**永远不**上报 `learning` / `code_editing` / `executing` / `body_switch`（§3.6 边界）
   - API-A 自主执行面 **永远不**上报 `body_switch`（§3.5 边界：身体切换由执行器执行）
   - 执行器**永远不**上报 `learning` / `code_editing`（§3.5 边界：学习由 API-A 自主执行面执行）
   ...
   **监督者空闲语义补充**：
   - 如果 API-B 判断位没有任务，监督者 scene 必须是 `idle` / 休息态；
   - 仅因为 API-A 自主执行面存在创造类待执行任务，不得把监督者误显示为 `planning`；
...
- `systems/self_learning/`
  - 旧 self-learning 结论 contract / store 与 Supervisor payload builder；不是自主任务执行服务
...
- **自主链路常驻运行**：Supervisor 默认随服务启动 review / endogenous-drive 循环；`/auto` 和 `/auto-q` 只接入或退出主 CLI 内本地观测/执行面，且不限制主 CLI 输入或用户与主 Agent 的交互
- **结构化四级记忆压缩**（Phase M5）：Event→Scene→Arc→Epoch 四级压缩管线已接入运行时
- **执行器收口为身体切换专用**：执行器已收缩为身体切换机械流程执行面，不做学习、不做升级
- **自主任务改为 API-A 自主执行面 pull 执行**：CLI `_poll_autonomous_workflow()` 与网关 `/v1/tasks/{task_id}/decision` 已接入正式任务链路；API-A 自主执行面会主动拉取并执行 `self_learning` / `body_improvement`。`/auto` 只是本地观测/执行面接入入口，不再是阻断用户交互的运行模式
- **治理在途存储已纳入 `body_improvement` 与 `running` 生命周期**：API-A 可正式回报 `running/completed/failed`，监督者也可对卡住任务执行正式失败清理
...
- **API-B 核**：Mem + 监督者，负责"记住自己是谁"、"当前局面是什么"、"该研究什么"、"哪些治理输出值得推动"和"该不该切换"。监督者管理治理在途存储，内生驱动先完成认知判断与治理输出选择，再把其中创造类兼容投影落成 `self_learning` 学习任务，证据成熟后再落成 `body_improvement` 改进任务。独立配置模型，不与 Agent 共享短期工作心智，也不能回环到 API-A chat 路由。
...
| **Governor Engine**（治理引擎）        | 内生驱动执行认知评估、形成治理输出、管理治理在途存储、整理记忆判断替身进展、裁决身体切换 | 当前实现前台已以 `judgement_core / attention_agenda / uncertainty_ledger / observation_program / meta_governance` 作为主判断结构；任务候选只保留为兼容投影，但内部仍存在一定 candidate-heavy 实现体量，程序继续负责输入裁剪、状态迁移、风险护栏与最终把关 |
...
目标语义下，监督者是全天候运行的治理身份：内生驱动、治理在途存储治理、记忆整理和替身进展判断按节律运行，不再依赖时间窗口或“等用户空闲”。用户链路始终走主 CLI / 主 Agent / `user_chat` 泳道；自主链路走 API-B 内生驱动 / 治理在途存储 / API-A 自主执行面 / `supervisor_task` 泳道；两者由 gateway 双槽隔离。

当前实现中，Supervisor 默认常驻自主链路；`/auto` 与 `/auto-q` 只控制主 CLI 内本地观测/执行面的接入状态：

| <br />             | 本地观测/执行面未接入 | 本地观测/执行面已接入 |
| ------------------ | ---------------------- | ---------------------- |
| health\_check loop | ✅                      | ✅                      |
| 结构化记忆维护            | ✅ 自适应周期                | ✅                      |
| 内生驱动器（认知评估 + 治理输出） | ✅ 按配置周期运行             | ✅ 按配置周期运行          |
| 治理在途存储（API-A 自主执行面遍历执行） | ✅ Supervisor 继续管理；本地执行面未接入时不显示 | ✅                      |
| 整理记忆 + 判断身体切换      | ✅ 按治理节律运行             | ✅                      |
| Gateway 用户请求       | ✅ 正常服务                 | ✅ 正常服务              |
| CLI 输入             | ✅ 正常                   | ✅ 正常                  |

这个表描述的是本地观测/执行面的接入状态，不是自主链路是否存在。`/auto` 不是用户链路模式，也不得影响主 CLI 输入。
...
也就是说，进入治理在途存储供 API-A 自主执行面通过自主任务执行通道遍历执行的，不只是单一的探索学习，而是"先学习、后改进"这条创造类兼容链路。其余判断优先在监督者内部完成；治理卫生、错误复核和候选清退本身，未来可以由 LM 监督者先提出治理动作，再交由程序护栏落地。
...
内生驱动器不产出身体切换任务。身体切换由监督者在整理记忆后内生判断替身进展，Governor 裁决建议切换（保有否决权），用户同意后由执行器执行激活（见 §7.5）。
...
API-A 自主执行面（最小迷你 CLI / 子代理通道；由主 CLI 内 `/auto` 接入本地观测/执行面）
  → 遍历治理在途存储
  → 拉取并执行 `self_learning` 任务（可自主决定是否使用子代理辅助）
  → 学习成果写入 Mem
```
...
```text
阶段 1 — 学习:
  监督者内生驱动 → self_learning 任务 → API-A 自主执行面执行 → 成果写入 Mem

阶段 2 — 替身改进 (学习质量 >= 60 分触发):
  监督者内生驱动 → body_improvement 任务 → API-A 自主执行面编辑 shell worktree
...
阶段 4 — 建议切换:
  health_score >= active_health + 15 或 shell > active
    ...
    → 批准后 → awaiting_user_consent
    → 用户同意后 → executor: shell→candidate→probe→active
```

**关键边界**：

- 学习任务由 API-A 自主执行面执行，可自主决定是否使用子代理辅助复杂任务
- 身体升级（代码编辑）由 API-A 自主执行面在 Git 替身基础上执行，不由执行器代劳
- 身体切换只由执行器执行机械流程
...
| 3  | **API-A 自主执行面执行自主任务，监督者管理治理在途存储** | `self_learning` 与 `body_improvement` 由监督者产出并放入治理在途存储，API-A 自主执行面通过自主任务执行通道遍历执行；主 CLI 的 `/auto` 只接入本地观测/执行面，用户主 CLI 不承担该执行槽 |
...
| 8  | **子代理是 API-A 的自主能力**    | 子代理不是被禁止的能力，也不是学习任务的强制模式。用户链路和自主链路中的 API-A 执行面都可按任务复杂度自主决定是否使用子代理辅助执行 |
| 9  | **用户服务绝对优先**            | 自主链路不能抢占用户链路；用户主 CLI / 主 Agent 与 API-A 自主执行面通过双槽隔离。`/auto` 只是本地观测/执行面接入命令，不得限制用户输入或主 Agent 交互 |
