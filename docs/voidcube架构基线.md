...
其中，**探索式学习任务**（创造类）进入任务列表供 Agent 在 AUTO / Governor Mode 下遍历执行，包括：
...
当学习成果累积质量达到阈值后，监督者还会把 **`body_improvement` 替身改进任务** 放入同一个任务列表，交给 Agent 在 shell 槽位 worktree 中执行代码改进。也就是说，任务列表承载的是"监督者放行给 Agent 执行的自主任务"，而不只是一类纯研究任务。
...
1. **所有任务统一展示**：监督者内生驱动四类候选、任务列表中的 Agent 自主任务（学习任务 + 替身改进任务）、身体切换任务——全部在同一任务面板中可见。当前 UI 仅展示前 5 条任务（`slice(0,5)`），远远不够。
2. **执行路径一目了然**：每条任务必须标注其执行路径，让观察者能区分：
   - **学习任务**：内生驱动（创造类）→ 任务列表 → Agent pull 执行
   - **替身改进**：监督者放行 `body_improvement` → 任务列表 → Agent 在 shell worktree 中编辑代码
   - **身体切换**：监督者裁决 → 执行器执行
...
4. **分类视图，非混乱堆叠**：任务按执行路径分组展示，不是简单的时间线堆砌。推荐分为以下面板：
   | 面板       | 内容                                            | 执行路径               |
   | -------- | --------------------------------------------- | ------------------ |
   | **学习任务** | 任务列表中 Agent 待执行/执行中/已完成的 `self_learning` 任务      | 监督者 → 任务列表 → Agent |
   | **替身改进** | 任务列表中 Agent 待执行/执行中/已完成的 `body_improvement` 任务 | 监督者 → 任务列表 → Agent |
...
- 身体切换状态在 UI 中不可见
- 内生驱动四类候选与任务列表中的 `self_learning` / `body_improvement` 任务混在一起，无法区分
...
- **API-B 核**：Mem + 监督者，负责"记住自己是谁"、"该研究什么"、"该不该继续保留某项任务"和"该不该切换"。监督者管理任务列表，内生驱动产出探索式学习任务，并应逐步升级为 LM 参与治理任务列表。独立配置模型，不与 Agent 共享短期工作心智，也不能回环到 API-A chat 路由。
...
其中，候选 3（创造类探索式学习任务）进入任务列表供 Agent 在 AUTO 模式下遍历执行。其余候选由监督者内部机制或记忆维护系统直接处理；但队列卫生、错误复核和候选清退本身，未来可以由 LM 监督者先提出治理动作，再交由程序护栏落地。
