...
| --- | --- | --- | --- | --- | --- | --- |
| API-B 内生驱动 → 任务列表 → API-A 自主执行体 → Mem → API-B 再读取 | **自主链路** | API-B（Mem / Supervisor）内生感知、判断、生成治理投影与任务 | API-A 域中的自主任务执行面；当前实现表现为主 CLI 内的最小迷你 CLI / AUTO executor，供 API-A 子代理执行 `self_learning` 与 `body_improvement` | Web 监控只展示 API-B 的状态、反馈、任务治理、内生驱动与由 API-A 回报给 API-B 的自主任务状态 | 学习成果、替身改进报告、任务 outcome 必须写入 Mem，并由下一轮 API-B 内生驱动读取和管理 | 不能接管用户主 CLI，不能把 Web 监控变成用户聊天面，不能让 AUTO 成为用户链路模式 |
...
`/auto` 与 `/auto-q` 只是当前过渡期保留的**自主链路启停开关**。它们不是主 CLI 的另一种用户交互模式，也不是用户链路是否可用的开关。目标形态下，自主链路应由 API-B 节律和治理条件自主运行，不需要用户手动 AUTO 才启动。
...
**关键纠正**：不存在独立的"自学系统"运行服务。学习任务的生产者是监督者（API-B），执行者是 API-A 自主执行面，学习成果的存储位置是 Mem。当前代码中 `systems/self_learning/` 的 `service.py` 等运行服务是过渡实现，应逐步收口：学习任务的生产归监督者，执行归 API-A 自主执行面，存储归 Mem；用户主 CLI 只属于用户链路，不承担自主任务执行槽。

### 3.6 监督者

监督者是 Mem 的提权治理身份，使用 API-B。监督者管理一个任务列表（即”任务管理器”），API-A 自主执行面通过自主任务执行通道遍历该列表执行监督者已放行的自主任务，其中包括 `self_learning` 和 `body_improvement`。当前实现里 AUTO 模式暂时保留为自主链路门控；它不是用户输入门控，也不是主 CLI 可用性的限制。
...
**当前实现临时状态**：`/auto` 与 `/auto-q` 仍暂时保留为自主链路启停门控，用于手动打开或停止 API-B 内生驱动到 API-A 自主执行面的全天候运行链路；但 AUTO 模式不再限制主 CLI 的用户输入，不会让 Gateway 用户请求返回 503，也不阻断用户使用主 Agent。待监督者门控完善后，应移除对 AUTO 命令的依赖，让自主链路无需手动启停即可按全天候语义运行。
...
任务列表（即"任务管理器"）不是独立的架构组件，而是由监督者直接管理的一个任务队列。监督者会把治理输出中适合交由 API-A 自主执行面执行的兼容任务投影放入此列表，API-A 自主执行面通过网关主动遍历此列表并执行。当前实现的 AUTO 只临时决定自主链路是否启用，不改变任务列表的架构归属。
...
- **内容**：包含 `self_learning` 与 `body_improvement` 两类自主任务；不包含 `body_switch` / `body_upgrade` 这类身体切换机械任务
...
- **API-A 自主执行卡**同一时刻只允许挂 1 个创造类任务（`self_learning` / `body_improvement`）；执行完成后任务回收消失，再从定时队列按先后顺序取下一个创造类任务
...
- `执行中`：任务已经离开定时队列，进入监督者执行卡或 API-A 自主执行卡
...
2. **任务卡固定分为 5 段，自上而下排列**：
   ...
   | **第 3 段** | API-A 自主执行卡 | 只展示当前被 API-A 自主执行面处理并回报给 API-B 的 1 个创造类任务 |
...
4. **第 2、3 段是单槽，不是列表**：
   ...
   - API-A 自主执行卡只保留最先处理的 1 个创造类任务；完成后消失回收，再从第 4 段定时队列按先后顺序补位
...
- **清理** **`systems/self_learning/service.py`** **等旧兼容面**：学习任务的生产归监督者，执行归 API-A 自主执行面，存储归 Mem；`SelfLearningService` 只可作为旧结论记录与 Supervisor payload 构造兼容层，不得再被解释为独立"自学系统"运行服务
...
- **自主任务改为 API-A 自主执行面 pull 执行**：CLI `_poll_auto_mode_workflow()` 与网关 `/v1/tasks/{task_id}/decision` 已接入正式任务链路；API-A 自主执行面会主动拉取并执行 `self_learning` / `body_improvement`。`AUTO` 只是当前自主链路临时启停入口，不再是阻断用户交互的运行模式
...
- 创造类任务进入定时队列后，由第 3 段 API-A 自主执行卡按先后顺序单槽消费
