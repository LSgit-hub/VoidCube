...
学习任务是监督者内生驱动器产出的探索式研究任务，是身体改造进化的证据来源。学习任务不是由独立的"自学系统"服务执行，而是由 API-A 的活跃 Agent 在 AUTO / Governor Mode 下直接执行。学习之后，如果监督者判断证据积累达到阈值，还会继续产出 `body_improvement` 任务，由同一个 API-A Agent 在 shell 槽位执行代码改进。

学习与改进任务的类型（由监督者内生驱动产出并管理）：
...
监督者的内生驱动器保持原有的三类核心价值观驱动能力，把”延续、真实、创造”映射为可审计的候选任务。**当前实现默认仍以确定性规则引擎（计数器 + 时间戳判断 + 状态机）为主；当 LLM API 凭据可用时，创造力候选（探索式学习任务）通过 LLM 分析压缩记忆上下文生成智能学习主题，`body_improvement` 的方向也可由 LLM 生成，替身改进完成后的 diff 质量也可由 LLM 审查。**无 LLM 时自动降级为确定性文本提取或历史启发式生成。

但目标基线应再前进一步：**监督者不应只是“用 LLM 生成内容”，还应逐步升级为“由 LM 参与治理任务列表本身”。**

也就是说，LM 驱动的监督者目标不是不停地产生任务扔进列表，而是结合长期记忆、最近任务结果、队列状态和用户服务优先级，去决定：
...
- **下层程序护栏**：做状态机、空闲窗口、执行窗口、边界限制、最终状态写入、超时失败和审计落盘
...
- **Governor Mode（`/auto`** **激活）**：启动内生驱动器，按配置周期（默认 300s）产出全部四类候选任务。其中会进入任务列表的，是交由 Agent 执行的自主任务：探索式学习任务（`self_learning`）以及达到阈值后产生的替身改进任务（`body_improvement`）。CLI `/auto-q` 退出回到 Memory Mode。
...
任务列表（即"任务管理器"）不是独立的架构组件，而是由监督者直接管理的一个任务队列。监督者的内生驱动器将交由 Agent 执行的自主任务放入此列表，Agent 在 AUTO / Governor Mode 下通过网关主动遍历此列表并执行。

任务列表的定位：

- **持有者**：监督者（属于 `systems/supervisor/` 的一部分，对应 `SelfEvolutionTaskQueue`）
- **生产者**：监督者内生驱动器（产出交由 Agent 执行的自主任务）
...
当前基线只保留两种调用 `auto` 模式的方式：

- **限时自动**：仅在 `00:00-06:00` 内允许自动执行
- **手动触发**：由用户明确触发 `auto`

当前基线**不再**以 CLI 空闲作为执行前提，也不再要求“连续 10 分钟无用户请求”这类额外闸门。执行窗口本身就是限时自动的唯一时间约束；窗口外若要执行，只能走手动触发。
...
- **同一预设时间只能保留一个任务**；与定时队列重复的候选任务不得再次进入定时队列
...
2. **任务卡固定分为 5 段，自上而下排列**：
   ...
   | **第 5 段** | 内生驱动候选列表 | 展示内生驱动每轮产出的候选任务列表，供监督者 LM 裁定是否进入定时队列 |
...
6. **第 5 段不是“只能放 1 个元素”的单卡**：它是“内生驱动生产的任务候选列表”。约束是“每次循环只产出 1 个新候选”，不是“整个列表 UI 只能展示 1 个”。候选列表中的任务不得与定时队列已有任务重复；若发生冲突，由监督者 LM 裁定。
...
- **API-B 核**：Mem + 监督者，负责"记住自己是谁"、"该研究什么"、"该不该继续保留某项任务"和"该不该切换"。监督者管理任务列表，内生驱动先产出 `self_learning` 学习任务，证据成熟后再产出 `body_improvement` 改进任务，并应逐步升级为 LM 参与治理任务列表。独立配置模型，不与 Agent 共享短期工作心智，也不能回环到 API-A chat 路由。
...
| **Governor Engine**（治理引擎）        | 内生驱动产出学习任务（LLM 优先生成智能主题）、管理任务列表、整理记忆判断替身进展、裁决身体切换 | 当前实现仍以确定性协议为主，LLM 主要参与主题生成、改进方向生成和 diff 审查；目标应升级为“LM 先给出整表治理建议，确定性协议最终把关” |
...
其中，候选 3（创造类）不是单一学习任务，而是分成两个连续分支：
...
也就是说，进入任务列表供 Agent 在 AUTO 模式下遍历执行的，不只是单一的探索学习，而是"先学习、后改进"这条创造类链路。其余候选由监督者内部机制或记忆维护系统直接处理；但队列卫生、错误复核和候选清退本身，未来可以由 LM 监督者先提出治理动作，再交由程序护栏落地。
...
- 内生驱动候选先进入第 5 段候选列表，经过监督者 LM 裁定后才进入第 4 段定时队列
- 第 5 段候选列表与第 4 段定时队列不得出现重复任务，尤其不得在同一 `scheduled_for` 时间保留多个冲突任务
...
| 14 | **LM 治理不能绕过程序护栏**        | 即使未来由 LM 参与任务列表治理，最终状态迁移、空闲窗口、执行窗口、边界检查和审计落盘仍必须由确定性程序负责 |
