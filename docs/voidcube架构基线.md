...
`/auto` 与 `/auto-q` 只是当前过渡期保留的**自主链路启停开关**。它们不是主 CLI 的另一种用户交互模式，也不是用户链路是否可用的开关。目标形态下，自主链路应由 API-B 节律和治理条件自主运行，不需要用户手动打开 `/auto` 才启动。
...
- **当前实现**：治理在途存储治理仍主要由 `evaluate_activity_guards()`、`review_autonomous_chain_tasks()`、`decide_autonomous_chain_task()` 和 `AutonomousChainStore.update_status()` 的规则、阈值和合法状态迁移控制
...
**当前实现临时状态**：`/auto` 与 `/auto-q` 仍暂时保留为自主链路启停门控，用于手动打开或停止 API-B 内生驱动到 API-A 自主执行面的全天候运行链路；但 `/auto` 开关不再限制主 CLI 的用户输入，不会让 Gateway 用户请求返回 503，也不阻断用户使用主 Agent。待监督者门控完善后，应移除对 `/auto` 命令的依赖，让自主链路无需手动启停即可按全天候语义运行。
...
- **当前临时门控：`/auto` 开关**：`/auto` 与 `/auto-q` 仍暂时作为自主链路启停开关，用于手动打开或停止 API-B 内生驱动到 API-A 自主执行面的全天候运行链路；该开关不限制主 CLI 输入，不影响用户与主 Agent 交互。待门控完善后，应移除对 `/auto` 命令的依赖。
...
**关键原则**：健康值达标是"建议"而非"自动"。Governor 保有否决权（程序前置门）；真正激活新替身（activate_slot）需**用户同意**（目标语义，待实现）。身体切换不由自主链路治理在途存储驱动。执行器只做切换的机械流程。
...
身体切换不由自主链路治理在途存储驱动。替身代码改进与 probe 验证由监督者/Agent 全天候自主进行；Governor 在收到健康值达标的建议切换事件后独立裁决（否决权为程序前置门）。**真正激活新替身（activate_slot）只能由用户同意后执行**（目标语义，待实现）——这是用户对"换身体"这一不可逆动作保有的最终控制权。执行器只做切换的机械流程，不做升级、不做学习、不做判断。
...
4. **任务与闭环分离**：
   ...
   - 时间线只展示“刚刚发生了什么”。
   三者不能混成旧式“任务总览 + 队列操作台”。
   - Web 小屋前端应优先消费专门的自主链路读模型，例如 `autonomous_observation.chain.segments / loop.stages / board.primary_focus` 一类投影，而不是直接把 Supervisor 底层任务存储结构当成页面协议。
5. **链路任务只做只读观测**：
...
- **自主链路临时门控**：`/auto` 和 `/auto-q` 目前仍可显式启停自主链路，但这只是过渡门控；目标语义是监督者全天候运行并驱动自主链路，且 `/auto` 不限制主 CLI 输入或用户与主 Agent 的交互
- **结构化四级记忆压缩**（Phase M5）：Event→Scene→Arc→Epoch 四级压缩管线已接入运行时
- **执行器收口为身体切换专用**：执行器已收缩为身体切换机械流程执行面，不做学习、不做升级
- **自主任务改为 API-A 自主执行面 pull 执行**：CLI `_poll_autonomous_workflow()` 与网关 `/v1/tasks/{task_id}/decision` 已接入正式任务链路；API-A 自主执行面会主动拉取并执行 `self_learning` / `body_improvement`。`/auto` 开关只是当前自主链路临时启停入口，不再是阻断用户交互的运行模式
...
当前实现仍保留 `/auto` 与 `/auto-q` 作为自主链路的临时启停开关：

| <br />             | `/auto` 关闭（临时停止自主链路） | `/auto` 开启（临时启用自主链路） |
| ------------------ | ---------------------- | ---------------------- |
| health\_check loop | ✅                      | ✅                      |
| 结构化记忆维护            | ✅ 自适应周期                | ✅                      |
| 内生驱动器（认知评估 + 治理输出） | 暂停                     | ✅ 每 300s              |
| 治理在途存储（API-A 自主执行面遍历执行） | 暂停                     | ✅                      |
| 整理记忆 + 判断身体切换      | 暂停                     | ✅                      |
| Gateway 用户请求       | ✅ 正常服务                 | ✅ 正常服务              |
| CLI 输入             | ✅ 正常                   | ✅ 正常                  |

这个表描述的是当前过渡开关，不是最终架构。最终应取消“必须手动 `/auto` 才运行自主链路”的限制。`/auto` 不是用户链路模式，也不得影响主 CLI 输入。
...
终端 dashboard / Web 小屋应优先消费 Supervisor 提供的 `autonomous_observation.chain.segments / loop.stages / board.primary_focus / board.hero_pills / board.observation_notes / board.recent_activity` 读模型；其中 `chain.segments` 与 `loop.stages` 是程序侧主协议，对应的治理 API 入口也应统一收口为 `/autonomous-chain/tasks`。观测板暂不可用时可以显示空态，但不应回退成旧 `/self-evolution/tasks` 人工任务台视角，也不应再从 `protocol_notes` 或 gateway 活动快照反推一套兼容说明。
...
对应地，Supervisor 的 `/ui/state` 不应再把顶层 `tasks`、`drive_candidates`，以及 `autonomous_observation.observed_tasks / autonomous_observation.candidates` 这类原始治理/候选切片作为前端主协议返回；前端应围绕 `autonomous_observation.chain.segments`、`loop.stages` 与 `board.primary_focus` 取数。

一旦 Supervisor 已经稳定投影出 `board.primary_focus / hero_pills / observation_notes / recent_activity`，Web 与最小 CLI 就不应再各自从 `loop.stages`、`protocol_notes` 或 gateway 活动快照反推一份“焦点/说明/最近动作”兼容视图；消费端应直接信任这组投影，避免同一闭环出现多套解释口径。

同理，`drive_available`、`autonomous_chain_gate` 这类 Web 观测专用运行时信号也应优先下沉到 `autonomous_observation.runtime` 一类读模型分支，而不是继续作为顶层 UI 镜像字段长期保留。
...
其中 `api_a_ready` 必须严格理解成“已放行、待 API-A 认领”的窗口，而不是 API-A 全部执行态总段。已经进入 `running` 的自主任务应继续通过 `loop.stages.api_a_execution`、最近事件和最近轨迹观察，不应再混回 `api_a_ready`，否则前端会重新退化成旧式任务台视角。
...
| 3  | **API-A 自主执行面执行自主任务，监督者管理治理在途存储** | `self_learning` 与 `body_improvement` 由监督者产出并放入治理在途存储，API-A 自主执行面通过自主任务执行通道遍历执行（当前由 `/auto` 临时启停）；用户主 CLI 不承担该执行槽 |
