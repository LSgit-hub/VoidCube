...
监督者的内生驱动器不应再被理解为单纯的“候选任务生成器”，而应被理解为**认知核心 + 治理输出中枢**。它依然保留“延续、真实、创造”三类核心价值观，但这些价值观首先进入的是感知、世界模型、need、intent、meta-governance 等认知层；**候选任务只是其中一条兼容输出投影**。当前实现默认仍以确定性规则引擎（计数器 + 时间戳判断 + 状态机）为主；当 LLM API 凭据可用时，创造力相关的学习主题、治理建议和 diff 审查仍可由 LLM 辅助生成或复核。无 LLM 时自动降级为确定性文本提取或历史启发式生成。
...
- **Governor Mode（`/auto`** **激活）**：启动内生驱动器，按配置周期（默认 300s）运行认知评估与治理输出；其中进入任务列表的，仍是交由 Agent 执行的自主任务：探索式学习任务（`self_learning`）以及达到阈值后产生的替身改进任务（`body_improvement`）。CLI `/auto-q` 退出回到 Memory Mode。
...
监督者内生驱动器将"延续、真实、创造"三类核心价值观先映射为认知判断与治理动作，再在当前阶段兼容投影为可审计候选项：
...
也就是说，进入任务列表供 Agent 在 AUTO 模式下遍历执行的，不只是单一的探索学习，而是"先学习、后改进"这条创造类链路。其余判断优先在监督者内部完成；队列卫生、错误复核和候选清退本身，未来可以由 LM 监督者先提出治理动作，再交由程序护栏落地。
...
- 内生驱动投影先进入第 5 段治理投影列表，经过监督者 LM 裁定后才进入第 4 段定时队列
