...
Copies bundled skills from the repo's skills/ directory into ~/.VoidCube/skills/
...
The manifest lives at ~/.VoidCube/skills/.bundled_manifest.
"""
...
def _get_bundled_dir() -> Path:
    ...
    Checks VOIDCUBE_BUNDLED_SKILLS env var first (set by Nix wrapper),
    then falls back to the relative path from this source file.
    """
    env_override = os.getenv("VOIDCUBE_BUNDLED_SKILLS")
    if env_override:
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.VoidCube/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.VoidCube/skills/ using the manifest.
    ...
    """
    try:
        from voidcube.extensions.skills.sync import sync_skills as canonical_sync
    except (ModuleNotFoundError, ImportError):
        from src.voidcube.extensions.skills.sync import sync_skills as canonical_sync
    return canonical_sync(quiet=quiet)

if __name__ == "__main__":
    print("Syncing bundled skills into ~/.VoidCube/skills/ ...")
