...
def _merge_warning(result_dict: dict, warning: str | None) -> None:
    """Append a warning to a tool result without dropping existing warnings."""
    if not warning:
        return
...
def _get_backend_warning(file_ops: ShellFileOperations) -> str | None:
    """Return any backend fallback warning attached to the active environment."""
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        resolved_candidate = runtime_path.host_path or runtime_path.backend_path

        # ── Binary file guard ─────────────────────────────────────────
        # Block binary files by extension (no I/O).
        if has_binary_extension(str(resolved_candidate)):
            _ext = Path(str(resolved_candidate)).suffix.lower()
            return json.dumps({
                "error": (
        ...
        _blocked_dirs = [
            ...
            _VoidCube_home / "skills" / ".hub",
        ]
        if runtime_path.host_path:
            _resolved = Path(runtime_path.host_path).expanduser().resolve(strict=False)
            for _blocked in _blocked_dirs:
                try:
                    _resolved.relative_to(_blocked)
                    return json.dumps({
                        "error": (
                            f"Access denied: {path} is an internal Voidcube cache file "
        ...
        resolved_str = runtime_path.tracking_path
        ...
        if cached_mtime is not None:
            if runtime_path.host_path:
                try:
                    current_mtime = os.path.getmtime(runtime_path.host_path)
        ...
        # ── Perform the read ──────────────────────────────────────────
        result = file_ops.read_file(runtime_path.backend_path, offset, limit)
        result_dict = result.to_dict()
        _merge_warning(result_dict, _get_backend_warning(file_ops))
        ...
        read_key = ("read", runtime_path.tracking_path, offset, limit)
        with _read_tracker_lock:
            ...
            if runtime_path.host_path:
                try:
                    _mtime_now = os.path.getmtime(runtime_path.host_path)
...
def _update_read_timestamp(filepath: str, task_id: str, file_ops: ShellFileOperations | None = None) -> None:
    ...
    """
    runtime_path = resolve_runtime_path(filepath, getattr(file_ops, "env", None))
    if not runtime_path.host_path:
        return
    try:
        current_mtime = os.path.getmtime(runtime_path.host_path)
    ...
    with _read_tracker_lock:
        ...
        if task_data is not None:
            task_data.setdefault("read_timestamps", {})[runtime_path.tracking_path] = current_mtime


def _check_file_staleness(filepath: str, task_id: str, file_ops: ShellFileOperations | None = None) -> str | None:
    ...
    with _read_tracker_lock:
        ...
        if not task_data:
            return None
        read_mtime = task_data.get("read_timestamps", {}).get(runtime_path.tracking_path)
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    try:
        ...
        stale_warnings = []
        for _p in _paths_to_check:
            _sw = _check_file_staleness(_p, task_id, file_ops=file_ops)
            if _sw:
        ...
        if mode == "replace":
            ...
            result = file_ops.patch_replace(resolved_paths[path].backend_path, old_string, new_string, replace_all)
        elif mode == "patch":
            if not patch:
        ...
        if not result_dict.get("error"):
            for _p in _paths_to_check:
                _update_read_timestamp(_p, task_id, file_ops=file_ops)
...
def search_tool(pattern: str, target: str = "content", path: str = ".",
    ...
    try:
        ...
        result = file_ops.search(
            pattern=pattern, path=runtime_path.backend_path, target=target, file_glob=file_glob,
...
registry.register(name="read_file", toolset="file", schema=READ_FILE_SCHEMA, handler=_handle_read_file, check_fn=_check_file_reqs, emoji="📖", max_result_size_chars=float('inf'), effect="read_only")
registry.register(name="write_file", toolset="file", schema=WRITE_FILE_SCHEMA, handler=_handle_write_file, check_fn=_check_file_reqs, emoji="✍️", max_result_size_chars=100_000, effect="idempotent_write")
registry.register(name="patch", toolset="file", schema=PATCH_SCHEMA, handler=_handle_patch, check_fn=_check_file_reqs, emoji="🔧", max_result_size_chars=100_000, effect="idempotent_write")
