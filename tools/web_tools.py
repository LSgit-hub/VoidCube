...
Backend is selected during ``VoidCube tools`` setup (web.backend in config.yaml).
When available, Voidcube can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``VoidCube tools``).
    Falls back to whichever API key is present for users who configured
    keys manually without running setup.
    
    支持 "local" 后端用于本地爬虫 (无需 API Key)。
    """
    configured = (_load_web_config().get("backend") or "").lower().strip()
    if configured in ("parallel", "firecrawl", "tavily", "exa", "local"):
        return configured
    
    # 本地模式检测: 检查 WEB_BACKEND 环境变量
    if os.getenv("WEB_BACKEND", "").lower() == "local":
        return "local"
...
def _get_firecrawl_client():
    ...
    Voidcube falls back to the Firecrawl tool-gateway for logged-in Nous Subscribers.
    """
...
def _resolve_web_extract_auxiliary(
    ...
    *,
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        ...
        extra_body = get_auxiliary_extra_body() or {"tags": ["product=VoidCube-agent"]}
...
def _get_exa_client():
    ...
    if _exa_client is None:
        ...
        _exa_client.headers["x-exa-integration"] = "VoidCube-agent"
    return _exa_client
    ...
    try:
        ...
        if backend == "local":
            try:
                ...
                logger.warning(f"本地爬虫模块未安装，使用简化搜索: {e}")
                return json.dumps({"success": False, "error": "Local search unavailable. Install beautifulsoup4 and lxml."})
...
async def web_extract_tool(
    ...
    async def _extract_with_local_backend(
        ...
        *,
        ...
        for url in target_urls:
            if _is_interrupted():
                local_results.append({"url": url, "error": "Interrupted", "title": ""})
                continue
            ...
            page = await asyncio.to_thread(local_web_extract, url)
    ...
    try:
        ...
        if not safe_urls:
            ...
            if backend == "local":
                results, backend_warning = await _extract_with_local_backend(safe_urls)
                ...
                if backend == "firecrawl" and not check_firecrawl_api_key():
                    backend_warning = (
                        "Configured backend 'firecrawl' is unavailable; fell back to local extraction."
                    )
                    logger.warning("%s", backend_warning)
                    results, backend_warning = await _extract_with_local_backend(
                        safe_urls,
        ...
        if backend_warning:
            trimmed_response["_warning"] = backend_warning
