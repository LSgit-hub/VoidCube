...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Voidcube-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_VOIDCUBE_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _VOIDCUBE_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_VOIDCUBE_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_VOIDCUBE_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _VOIDCUBE_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
