"""Base class for all Voidcube execution environment backends.
...
def cleanup_old_temp_files():
    """Clean up old VoidCube temporary files on startup.
    
    Removes:
    - VoidCube-cwd-*.txt files older than 24 hours
    - VoidCube-snap-*.sh files older than 24 hours
    """
    import tempfile
    import time
    from pathlib import Path
    ...
    for env_var in ("TMPDIR", "TMP", "TEMP"):
        ...
        if candidate and os.path.isdir(candidate):
            temp_dirs.append(Path(candidate))
    if os.path.isdir("/tmp"):
        temp_dirs.append(Path("/tmp"))
    try:
        temp_dirs.append(Path(tempfile.gettempdir()))
    except Exception:
        pass
    ...
    for temp_dir in temp_dirs:
        ...
        # Clean up old cwd files
        for pattern in ("VoidCube-cwd-*.txt", "VoidCube-snap-*.sh"):
            try:
                for f in temp_dir.glob(pattern):
                    try:
                        file_mtime = f.stat().st_mtime
                        if current_time - file_mtime > one_day:
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {VOIDCUBE_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_VoidCube_home() / "sandboxes"
...
def _build_bash_command(cmd: list[str], cmd_string: str, login: bool = False, quote: bool = False) -> list[str]:
    """Build a bash command list with proper login shell handling.

    Appends either ``["bash", "-l", "-c", cmd_string]`` (login=True) or
    ``["bash", "-c", cmd_string]`` (login=False) to the given command list.

    Args:
        cmd: Base command list to extend
        cmd_string: The bash command string to execute
        login: Whether to use login shell (-l flag)
        quote: Whether to quote the cmd_string using shlex.quote
    ...
    import shlex
    if quote:
        cmd_string = shlex.quote(cmd_string)
...
def _cwd_marker(session_id: str) -> str:
    return f"__VOIDCUBE_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Voidcube backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/VoidCube-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/VoidCube-cwd-{self._session_id}.txt"
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        parts.append("exit $__VoidCube_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"VOIDCUBE_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __VOIDCUBE_CWD_{session}__ marker from stdout output.
