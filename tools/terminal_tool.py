...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Voidcube exit
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("VOIDCUBE_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (VOIDCUBE_INTERACTIVE=1).
    ...
    try:
        os.environ["VOIDCUBE_SPINNER_PAUSE"] = "1"
        ...
        if "VOIDCUBE_SPINNER_PAUSE" in os.environ:
            del os.environ["VOIDCUBE_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (VOIDCUBE_INTERACTIVE=1):
    ...
    if not has_configured_password and not sudo_password and os.getenv("VOIDCUBE_INTERACTIVE"):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.VoidCube/.env or environment variables."
        )
...
def _annotate_environment_backend(
    env: Any,
    requested_env_type: str,
    active_env_type: str,
    warning: Optional[str] = None,
):
    """Attach backend metadata to an environment instance."""
    setattr(env, "_voidcube_requested_backend", requested_env_type)
    setattr(env, "_voidcube_active_backend", active_env_type)
    setattr(env, "_voidcube_backend_warning", warning)
    return env


def _local_fallback_cwd(host_cwd: str | None) -> str:
    """Choose a safe host cwd when sandbox creation falls back to local."""
    if host_cwd:
        candidate = os.path.abspath(os.path.expanduser(host_cwd))
...
def _create_environment_once(env_type: str, image: str, cwd: str, timeout: int,
    ...
    Args:
        ...
        network: Whether container backends should have network access
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "VOIDCUBE_ENABLE_NOUS_MANAGED_TOOLS is not enabled and no direct "
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
                        ...
                        network: bool = True):
    """Create an execution environment, optionally falling back to local."""
    try:
        env = _create_environment_once(
            env_type=env_type,
            image=image,
            cwd=cwd,
            timeout=timeout,
        ...
        return _annotate_environment_backend(env, env_type, env_type)
    except Exception as primary_error:
        if env_type == "local" or not fallback_to_local:
            raise

        warning = (
            f"Requested backend '{env_type}' is unavailable; fell back to local. "
        ...
        try:
            env = _create_environment_once(
                env_type="local",
                image="",
                cwd=_local_fallback_cwd(host_cwd),
                timeout=timeout,
                ssh_config=None,
            ...
            raise RuntimeError(
                f"{warning}. Local fallback also failed: "
                f"{type(fallback_error).__name__}: {fallback_error}"
            ) from fallback_error

        return _annotate_environment_backend(env, env_type, "local", warning)
...
def terminal_tool(
    ...
    try:
        ...
        elif requested_env_type == "podman":
            image = overrides.get("podman_image") or config["podman_image"]
        elif requested_env_type == "singularity":
            image = overrides.get("singularity_image") or config["singularity_image"]
        elif requested_env_type == "modal":
            image = overrides.get("modal_image") or config["modal_image"]
        elif requested_env_type == "daytona":
        ...
        if needs_creation:
            ...
            with task_lock:
                ...
                if needs_creation:
                    if requested_env_type == "singularity":
                        _check_disk_usage_warning()
                    logger.info("Creating new %s environment for task %s...", requested_env_type, effective_task_id[:8])
                    try:
                        ssh_config = None
                        if requested_env_type == "ssh":
                            ssh_config = {
                        ...
                        container_config = None
                        if requested_env_type in ("docker", "podman", "singularity", "modal", "daytona"):
                            container_config = {
        ...
        if background:
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    logger.error("Execution failed after %d retries - Command: %s - Error: %s: %s - Task: %s, Backend: %s",
                                 max_retries, _safe_command_preview(command), type(e).__name__, e, effective_task_id, active_env_type)
                    return json.dumps({
                        "output": "",
                        "exit_code": -1,
            ...
            if requested_env_type != active_env_type:
                result_dict["requested_backend"] = requested_env_type
                result_dict["active_backend"] = active_env_type
            if backend_warning:
                result_dict["_warning"] = backend_warning
...
if __name__ == "__main__":
    ...
    print(f"  TERMINAL_PODMAN_IMAGE: {os.getenv('TERMINAL_PODMAN_IMAGE', default_img)}")
