"""
Timezone-aware clock for Voidcube.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``VOIDCUBE_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.VoidCube/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Voidcube never
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("VOIDCUBE_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
