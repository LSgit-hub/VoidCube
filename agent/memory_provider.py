...
class MemoryProvider(ABC):
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'VoidCube memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], VoidCube_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'VoidCube memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``VoidCube_home`` is the active VOIDCUBE_HOME directory path.
