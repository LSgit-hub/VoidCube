...
test('opens the supervisor and a real VoidCube PTY', async () => {
  ...
  try {
    ...
    await expect(window.locator('.plugin-row').filter({ hasText: '目标管理器' })).toHaveAttribute('data-plugin-open', 'goal_manager')
    ...
    await window.locator('.plugin-row').filter({ hasText: '目标管理器' }).click()
