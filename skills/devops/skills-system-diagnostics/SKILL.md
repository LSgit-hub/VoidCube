...
（本例 skills-system-diagnostics 一度 19 节、337 行）。

步骤：

1. 先按标题切块：`[i for i, l in enumerate(lines) if l.startswith("## ")]`，**只用精确区间取块**。
   - **别用"从某标题取到文件末尾"**：后面不相关的章节会被一并带进来。本例因此把
     "索引去重与优先级"和"旧版孤儿注册表"重复塞进新结构，必须再跑一次清理（337 -> 310 行）。
2. 改写前**断言**所有预期章节标题都在源文件里（多写者环境随时可能已变动），缺任何一个就 abort。
3. 合并同类章节时**逐条核对事实标记**（如 `_bundled_dir` / `eol=lf` / `priority` /
   `seen_skill_names` / 死变量 / `registry-orphan`），确保瘦身不丢事实。
4. 一次写入（读 -> 拼 -> 写），写完立即复验：重复标题数 == 0、行数下降、事实标记齐全、
   二级标题集合符合预期。
