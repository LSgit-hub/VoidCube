...
在生产系统中使用 Outlines 进行结构化生成的实际示例。
...
def rate_limited_extract(texts: list[str], schema: type[BaseModel]):
    """带速率限制的提取。"""
    limiter = RateLimiter(max_requests=10, time_window=60)  # 10 请求/分钟
    model = outlines.models.transformers("microsoft/Phi-3-mini-4k-instruct")
    generator = outlines.generate.json(model, schema)
...
- **GitHub 示例**：https://github.com/outlines-dev/outlines/tree/main/examples
