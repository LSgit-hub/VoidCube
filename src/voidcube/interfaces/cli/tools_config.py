...
from .cli_output import (  # noqa: E402 — late import block
...
def _visible_providers(cat: dict, config: dict) -> list[dict]:
    ...
    return visible_providers(
        cat,
        features=get_nous_subscription_features(config),
        managed_tools_enabled=managed_nous_tools_enabled,
    )
...
def _configure_mcp_tools_interactive(config: dict):
    """Compatibility adapter for the canonical MCP configuration UI."""
    from .curses_ui import curses_checklist
    try:
    ...
    configure_mcp_tools(
        config,
        checklist=curses_checklist,
        probe_tools=probe_mcp_server_tools,
        ...
        print_line=print,
        style=color,
        dim_style=Colors.DIM,
        yellow_style=Colors.YELLOW,
        green_style=Colors.GREEN,
    )
