"""``VoidCube logs`` — view and filter Voidcube log files.
...
Usage examples::
    ...
    VoidCube logs                    # last 50 lines of agent.log
    VoidCube logs -f                 # follow agent.log in real time
    VoidCube logs errors             # last 50 lines of errors.log
    VoidCube logs gateway -n 100    # last 100 lines of gateway.log
    VoidCube logs --level WARNING    # only WARNING+ lines
    VoidCube logs --session abc123   # filter by session ID substring
    VoidCube logs --component tools  # only tool-related lines
    VoidCube logs --since 1h         # lines from the last hour
    VoidCube logs --since 30m -f     # follow, starting 30 min ago
"""
...
from ...infrastructure.config.runtime_paths import get_VoidCube_home, display_VoidCube_home
...
# Matches: "INFO gateway.run:" or "INFO [sess_abc] voidcube.infrastructure.execution.terminal_tool:"
...
def tail_log(
    ...
    *,
    ...
    log_path = get_VoidCube_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Voidcube runs — try 'VoidCube chat' first)")
        sys.exit(1)
    ...
    if component:
        from ...infrastructure.observability.logging import COMPONENT_PREFIXES
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_VoidCube_home()}/logs/")
        return

    print(f"Log files in {display_VoidCube_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'VoidCube chat' to generate logs)")
