"""
MCP Server Management CLI — ``VoidCube mcp`` subcommand.
...
from ...infrastructure.config.runtime_paths import display_VoidCube_home, get_VoidCube_home  # noqa: F401
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from .cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  VoidCube mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  VoidCube mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  VoidCube mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_VoidCube_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: VoidCube mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_VoidCube_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  VoidCube mcp add <name> --url <endpoint>')
        _info('  VoidCube mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
# ─── VoidCube mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'VoidCube mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``VoidCube mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("VoidCube mcp serve                              Run as MCP server")
        _info("VoidCube mcp add <name> --url <endpoint>        Add an MCP server")
        _info("VoidCube mcp add <name> --command <cmd>         Add a stdio server")
        _info("VoidCube mcp add <name> --preset <preset>       Add from a known preset")
        _info("VoidCube mcp remove <name>                      Remove a server")
        _info("VoidCube mcp list                               List servers")
        _info("VoidCube mcp test <name>                        Test connection")
        _info("VoidCube mcp configure <name>                   Toggle tools")
        print()
