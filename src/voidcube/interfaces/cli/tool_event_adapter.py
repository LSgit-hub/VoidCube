    ...
    *,
    ...
    if getattr(host, "_inline_diffs_enabled", False):
        ...
        if not host._should_emit_scrollback_output():
            ...
            try:
                from ...runtime.agent.display import render_edit_diff_with_delta
...
def _project_started(
    ...
    *,
    ...
    if getattr(host, "_inline_diffs_enabled", False):
        try:
            from ...runtime.agent.display import capture_local_edit_snapshot
    ...
    if event.name and not event.name.startswith("_"):
        from ...runtime.agent.display import get_tool_emoji, get_tool_preview_max_len
