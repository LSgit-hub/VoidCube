...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if pool_present:
            ...
            return _client_factory.create_openai_client(api_key, base_url, **extra), model
...
def _try_openrouter() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if pool_present:
        ...
        return _client_factory.create_openai_client(
            or_key, base_url, default_headers=_OR_HEADERS
        ), model
    ...
    return _client_factory.create_openai_client(
        or_key, OPENROUTER_BASE_URL, default_headers=_OR_HEADERS
    ), model
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    return (
        _client_factory.create_openai_client(_nous_api_key(nous), base_url),
        model,
    )
...
def _read_main_model() -> str:
    ...
    """
    try:
        from ..config.configuration import get_active_model_config, load_config
        cfg = load_config()
...
def _read_main_provider() -> str:
    ...
    """
    try:
        from ..config.configuration import get_active_provider_key, load_config
        cfg = load_config()
...
def _try_custom_endpoint() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    return _client_factory.create_openai_client(custom_key, custom_base), model
...
def _get_provider_chain() -> List[tuple]:
    return _auxiliary_fallback.provider_chain(
        (
...
def _fallback_chain_label(provider: str) -> str:
    try:
        ...
        _get_named_custom_provider = None
    return _auxiliary_fallback.fallback_chain_label(
        provider,
        named_custom_provider=_get_named_custom_provider,
    )
...
def _try_provider_fallback(
    ...
    try:
        ...
        _get_named_custom_provider = None
    return _auxiliary_fallback.try_provider_fallback(
        failed_provider,
        task=task,
        reason=reason,
...
def _to_async_client(sync_client, model: str):
    ...
    return _client_factory.create_async_openai_client(
        sync_client.api_key,
        str(sync_client.base_url),
        model,
    ...
    if provider == "custom":
        if explicit_base_url:
            ...
            if "api.kimi.com" in custom_base.lower():
                extra["default_headers"] = {"User-Agent": "KimiCLI/1.30.0"}
            client = _client_factory.create_openai_client(
                custom_key, custom_base, **extra
            )
            return (_to_async_client(client, final_model) if async_mode
                    else (client, final_model))
        ...
        if custom_entry:
            ...
            if custom_base:
                ...
                if not final_model:
                    ...
                    return None, None
                client = _client_factory.create_openai_client(custom_key, custom_base)
                logger.debug(
...
_VISION_AUTO_PROVIDER_ORDER = _auxiliary_vision.VISION_AUTO_PROVIDER_ORDER
...
def get_configured_vision_backends() -> List[str]:
    ...
    return _auxiliary_vision.configured_backends(
        requested=requested,
        base_url=base_url,
        main_provider=_read_main_provider(),
        provider_configured=_vision_provider_configured,
        normalize_provider=_normalize_vision_provider,
    )


def get_available_vision_backends() -> List[str]:
    return _auxiliary_vision.available_backends(
        main_provider=_read_main_provider(),
        main_model=_read_main_model(),
        strict_available=_strict_vision_backend_available,
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    return _resolve_vision_client_policy(
        ...
        main_provider=_read_main_provider(),
        main_model=_read_main_model(),
        provider_order=_VISION_AUTO_PROVIDER_ORDER,
        strict_backend=_resolve_strict_vision_backend,
        resolve_provider_client=resolve_provider_client,
        get_cached_client=lambda selected, selected_model, selected_async=False: _get_cached_client(
            selected, selected_model, selected_async
        ),
        to_async_client=_to_async_client,
        log=logger,
    )
...
_client_cache_manager = AuxiliaryClientCache()
_client_cache = _client_cache_manager.entries
_client_cache_lock = _client_cache_manager.lock
...
def _force_close_async_httpx(client: Any) -> None:
    ...
    """
    _force_close_cached_httpx(client)
...
def shutdown_cached_clients() -> None:
    ...
    """
    _client_cache_manager.shutdown()
...
def cleanup_stale_async_clients() -> None:
    ...
    """
    _client_cache_manager.cleanup_stale_async()
...
def _get_cached_client(
    ...
    """
    return _client_cache_manager.get_or_create(
        provider,
        model=model,
        async_mode=async_mode,
        base_url=base_url,
        api_key=api_key,
        main_runtime=main_runtime,
        resolve_client=resolve_provider_client,
        normalize_runtime=_normalize_main_runtime,
        runtime_fields=tuple(_MAIN_RUNTIME_FIELDS),
        compat_model=_compat_model,
    )
...
_is_connection_error = _auxiliary_policy.is_connection_error
_fallback_reason = _auxiliary_policy.fallback_reason
_normalize_vision_provider = _auxiliary_policy.normalize_vision_provider
_resolve_task_provider_model = _auxiliary_policy.resolve_task_provider_model
...
def _resolve_auxiliary_call_target(
    *,
    ...
    """Compatibility adapter around the canonical target orchestrator."""
    return _auxiliary_orchestration.resolve_call_target(
        task=task,
        provider=provider,
        model=model,
        base_url=base_url,
        ...
        require_active_integration=require_active_integration,
        resolve_vision_provider_client=resolve_vision_provider_client,
        get_cached_client=_get_cached_client,
        infer_active_provider=_infer_active_provider,
        missing_provider_error=_missing_provider_error,
        log=logger,
    )
...
def _execute_sync_auxiliary_call(
    *,
    ...
    return _auxiliary_execution.execute_sync(
        ...
        timeout=timeout,
        extra_body=extra_body,
        validate_response=_validate_llm_response,
        retry_kwargs=_completion_token_retry_kwargs,
        fallback_reason=_fallback_reason,
        prepare_fallback=_prepare_fallback_call,
    )
...
async def _execute_async_auxiliary_call(
    *,
    ...
    """Compatibility adapter around the canonical async execution policy."""
    return await _auxiliary_execution.execute_async(
        target=target,
        task=task,
        kwargs=kwargs,
        messages=messages,
