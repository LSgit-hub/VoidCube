...
from ...domain.contracts.integration_policy import contains_retired_integration
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    try:
        from .auth import resolve_nous_runtime_credentials
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``voidcube.infrastructure.providers.auth.resolve_provider()`` to resolve it to a concrete
...
def resolve_fast_mode_overrides(model_id: Optional[str]) -> dict[str, Any] | None:
    ...
    ``build_chat_completion_kwargs`` in ``voidcube.infrastructure.llm.request``.
    """
