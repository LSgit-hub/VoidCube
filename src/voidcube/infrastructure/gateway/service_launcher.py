...
try:
    ...
    load_VoidCube_dotenv(project_env=Path(__file__).resolve().parents[4] / ".env")
except Exception:
    pass
...
SERVICES: Dict[str, ServiceInfo] = {
    "gateway": ServiceInfo(
        name="gateway",
        port=GATEWAY_PORT,
        module="voidcube.infrastructure.gateway.internal_gateway:InternalGateway",
    ...
    "supervisor": ServiceInfo(
        ...
        module="voidcube.systems.supervisor.supervisor:Supervisor",
...
def find_gateway_pids() -> list[int]:
    """Return live gateway process IDs from the service record and port owner."""
    service = SERVICES["gateway"]
    candidates: list[int] = []
    pid = _read_pid(service.pid_file)
    if pid is not None and _pid_alive(pid):
        candidates.append(pid)
    owner = _port_owner_pid(service.port)
...
def get_service_name() -> str:
    """Return the user-level service manager name for the gateway."""
    return "VoidCube-gateway"


def get_launchd_label() -> str:
    """Return the launchd label used by the macOS gateway service."""
    return "com.voidcube.gateway"
