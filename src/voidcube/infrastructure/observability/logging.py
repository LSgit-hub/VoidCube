"""Centralized logging setup for Voidcube Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.VoidCube/logs/`` (profile-aware via ``get_VoidCube_home()``).
...
from ..config.runtime_paths import get_config_path, get_VoidCube_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_VoidCube_session_injector", False):
    ...
    _session_record_factory._VoidCube_session_injector = True  # type: ignore[attr-defined]
...
def setup_logging(
    *,
    VoidCube_home: Optional[Path] = None,
    ...
    """Configure the Voidcube logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    VoidCube_home
        Override for the Voidcube home directory.  Falls back to
        ``get_VoidCube_home()`` (profile-aware).
    log_level
    ...
    from ..persistence.redaction import RedactingFormatter
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_VoidCube_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._VoidCube_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the VoidCube group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            from ..config.configuration import is_managed
...
def _read_logging_config():
    ...
    try:
        ...
        # Config file may not exist, be unreadable, or contain invalid YAML
        pass
