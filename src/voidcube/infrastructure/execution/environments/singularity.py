...
_SNAPSHOT_STORE = get_VoidCube_home() / "singularity_snapshots.json"
...
def _ensure_singularity_available() -> str:
    ...
    try:
        result = subprocess.run(
            [exe, "version"], capture_output=True, text=True, errors="replace", timeout=10,
        )
...
def _get_scratch_dir() -> Path:
    ...
    if scratch.exists() and os.access(scratch, os.W_OK):
        user_scratch = scratch / os.getenv("USER", "VoidCube") / "VoidCube-agent"
    ...
    with _sif_build_lock:
        ...
        try:
            result = subprocess.run(
                ...
                capture_output=True, text=True, errors="replace", timeout=600, env=env,
            )
            if result.returncode != 0:
...
class SingularityEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        self.instance_id = f"VoidCube_{uuid.uuid4().hex[:12]}"
        ...
        if self._persistent:
            overlay_base = _get_scratch_dir() / "VoidCube-overlays"
    ...
    def _start_instance(self):
        ...
        try:
            from ..credential_files import get_credential_file_mounts, get_skills_directory_mount
