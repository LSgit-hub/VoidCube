...
_FORCE_SYNC_ENV = "VOIDCUBE_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.VoidCube") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.VoidCube to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.VoidCube", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``VOIDCUBE_FORCE_FILE_SYNC=1`` is set.
