...
from .base import BaseEnvironment, _popen_bash, _build_bash_command
...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "VoidCube-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.VoidCube"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.VoidCube directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.VoidCube"
        dirs = [base, f"{base}/skills", f"{base}/credentials", f"{base}/cache"]
        cmd = self._build_ssh_command()
        cmd.append(quoted_mkdir_command(dirs))
        subprocess.run(cmd, capture_output=True, text=True, errors="replace", timeout=10)
    ...
    def _scp_upload(self, host_path: str, remote_path: str) -> None:
        ...
        subprocess.run(mkdir_cmd, capture_output=True, text=True, errors="replace", timeout=10)
        ...
        result = subprocess.run(scp_cmd, capture_output=True, text=True, errors="replace", timeout=30)
        if result.returncode != 0:
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="VoidCube-ssh-bulk-") as staging:
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        cmd = self._build_ssh_command()
        _build_bash_command(cmd, cmd_string, login, quote=True)
