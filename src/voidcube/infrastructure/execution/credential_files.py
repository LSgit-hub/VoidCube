...
def register_credential_file(
    ...
    *relative_path* is relative to ``VOIDCUBE_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside VOIDCUBE_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    VoidCube_home = _resolve_VoidCube_home()

    # Reject absolute paths — they bypass the VOIDCUBE_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to VOIDCUBE_HOME)",
            relative_path,
        )
        return False

    host_path = VoidCube_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape VOIDCUBE_HOME.
    from ...extensions.tools.files.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, VoidCube_home)
...
def _load_config_files() -> List[Dict[str, str]]:
    ...
    try:
        from ..config.configuration import read_raw_config
        VoidCube_home = _resolve_VoidCube_home()
        cfg = read_raw_config()
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.VoidCube/{rel}"
                        result.append({
...
def get_skills_directory_mount(
    ...
    **Security:** Bind mounts follow symlinks, so a malicious symlink inside
    ...
    try:
        from ...extensions.skills.catalog import get_external_skills_dirs
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="VoidCube-skills-safe-"))
...
def get_cache_directory_mounts(
    ...
    ``cache/<name>`` layout.
    """
    from ..config.runtime_paths import get_cache_dir
