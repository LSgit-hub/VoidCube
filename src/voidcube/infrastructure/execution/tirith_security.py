...
def _get_VoidCube_home() -> str:
    """Return the Voidcube home directory, respecting VOIDCUBE_HOME env var."""
    return str(get_VoidCube_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_VoidCube_home(), ".tirith-install-failed")
...
def _VoidCube_bin_dir() -> str:
    """Return $VOIDCUBE_HOME/bin, creating it if needed."""
    d = os.path.join(_get_VoidCube_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $VOIDCUBE_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_VoidCube_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $VOIDCUBE_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $VOIDCUBE_HOME/bin/tirith
    ...
    VoidCube_bin = os.path.join(_VoidCube_bin_dir(), "tirith")
    if os.path.isfile(VoidCube_bin) and os.access(VoidCube_bin, os.X_OK):
        _resolved_path = VoidCube_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return VoidCube_bin
