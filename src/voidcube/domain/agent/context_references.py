...
_SENSITIVE_VOIDCUBE_DIRS = (Path("skills") / ".hub",)
...
async def _default_url_fetcher(url: str) -> str:
    from ...extensions.tools.web.web_tools import web_extract_tool
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    blocked_exact.add(VoidCube_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(VoidCube_home / rel for rel in _SENSITIVE_VOIDCUBE_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Voidcube path and cannot be attached")
