...
Uses a progressive-disclosure architecture:
...
# All skills live in ~/.VoidCube/skills/ (seeded from bundled skills/ on install).
# This is the single source of truth -- agent edits, hub installs, and bundled
# skills all coexist here without polluting the git repo.
VOIDCUBE_HOME = get_VoidCube_home()
SKILLS_DIR = VOIDCUBE_HOME / "skills"

# Limits for progressive disclosure efficiency
MAX_NAME_LENGTH = 64
...
_EXCLUDED_SKILL_DIRS = frozenset((".git", ".github", ".hub", "_disabled"))
...
def _is_gateway_surface() -> bool:
    if os.getenv("VOIDCUBE_GATEWAY_SESSION"):
        return True
    return bool(os.getenv("VOIDCUBE_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    return "Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to ~/.VoidCube/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.VoidCube/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.VoidCube/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``VoidCube skills`` config UI). Default False
        ...
        for scan_dir in all_dirs:
            for skill_md in scan_dir.rglob("SKILL.md"):
                ...
                try:
                    ...
                    logger.debug("Failed to parse SKILL.md at %s: %s", skill_md, e)
                    frontmatter = {}
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": "No skills found. Skills directory created at ~/.VoidCube/skills/.",
                },
                ensure_ascii=False,
            )
...
def skill_view(name: str, file_path: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        # (local skills dir + bundled repo skills + configured external dirs are all trusted)
        _outside_skills_dir = True
        _trusted_dirs = []
        try:
            _trusted_dirs.extend(d.resolve() for d in all_dirs)
        except OSError as e:
            logger.debug("Error resolving trusted directory: %s", e)
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.VoidCube/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `VoidCube skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
