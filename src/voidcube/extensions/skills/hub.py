#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Voidcube Skills Hub.
...
SKILLS_DIR = VOIDCUBE_HOME / "skills"
...
# Stable data contracts live in the canonical extensions package.  Keep these
# names here so existing source adapters and CLI commands remain importable.
try:
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            VoidCube_meta = metadata.get("VoidCube", {})
            if isinstance(VoidCube_meta, dict):
                tags = VoidCube_meta.get("tags", [])
        if not tags:
    ...
    def _discover_identifier(self, identifier: str, detail: Optional[dict] = None) -> Optional[str]:
        ...
        for base_path in base_paths:
            try:
                ...
                logger.debug("Failed to list skills in repo %s at %s: %s", repo, base_path, e)
                continue
            for meta in skills:
                ...
                if isinstance(entries, list):
                    for entry in entries:
                        ...
                        try:
                            ...
                            logger.debug("Failed to list skills in repo %s at %s/: %s", repo, dir_name, e)
                            continue
                        for meta in skills:
                            if self._matches_skill_tokens(meta, tokens):
                                return meta.identifier
        except Exception as e:
            logger.debug("Error during brute-force search in repo %s: %s", repo, e)
            pass
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.VoidCube/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                VoidCube_meta = meta_block.get("VoidCube", {})
                if isinstance(VoidCube_meta, dict):
                    tags = VoidCube_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Voidcube centralized index source
# ---------------------------------------------------------------------------

VOIDCUBE_INDEX_URL = "https://VoidCube-agent.nousresearch.com/docs/api/skills-index.json"
VOIDCUBE_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "VoidCube-index.json"
VOIDCUBE_INDEX_TTL = 6 * 3600  # 6 hours


def _load_VoidCube_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for VOIDCUBE_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if VOIDCUBE_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - VOIDCUBE_INDEX_CACHE_FILE.stat().st_mtime
            if age < VOIDCUBE_INDEX_TTL:
                return json.loads(VOIDCUBE_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(VOIDCUBE_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Voidcube index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Voidcube index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        VOIDCUBE_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        VOIDCUBE_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class VoidcubeIndexSource(SkillSource):
    """Skill source backed by the centralized Voidcube Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_VoidCube_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "VoidCube-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "VoidCube-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        VoidcubeIndexSource(auth=auth), # Centralized index (search + resolved install paths)
        ...
        try:
            for fut in as_completed(futures, timeout=overall_timeout):
                try:
                    ...
                    logger.debug("Error fetching results from source %s: %s", sid, e)
                    pass
        except TimeoutError:
            timed_out_ids = [
