...
discovers their tools, and registers them into the VoidCube-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.VoidCube/config.yaml under the ``mcp_servers`` key.
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            VoidCube_home = os.path.expanduser(
                os.getenv(
                    "VOIDCUBE_HOME", os.path.join(os.path.expanduser("~"), ".VoidCube")
                )
            )
            candidates = [
                os.path.join(VoidCube_home, "node", "bin", resolved_command),
...
class MCPServerTask:
    ...
    async def _refresh_tools(self):
        ...
        async with self._refresh_lock:
            ...
            # 2. Remove old tools from VoidCube-* umbrella toolsets
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Voidcube config file.
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Voidcube's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Voidcube registry schema format.
...
def _sync_mcp_toolsets(server_names: Optional[List[str]] = None) -> None:
    """Expose each MCP server as a standalone toolset and inject into VoidCube-* sets.
    ...
    Also injects all MCP tools into VoidCube-* umbrella toolsets for the
    default behavior.
    ...
    # Also inject into VoidCube-* umbrella toolsets for default behavior.
    for ts_name, ts in TOOLSETS.items():
        if not ts_name.startswith("VoidCube-"):
            continue
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> List[str]:
    ...
    and VoidCube-* umbrella toolset injection.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``VoidCube tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Voidcube registry.
