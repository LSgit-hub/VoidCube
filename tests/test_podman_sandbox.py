...
from voidcube.infrastructure.execution import podman_sandbox
...
def test_build_image_uses_packaged_containerfile(monkeypatch):
    ...
    assert command[5].endswith(
        "voidcube\\infrastructure\\execution\\containerfiles\\podman-agent.Containerfile"
    ) or command[5].endswith(
        "voidcube/infrastructure/execution/containerfiles/podman-agent.Containerfile"
    )
...
def test_inspect_image_digest_requires_immutable_id(monkeypatch):
    ...
    from voidcube.infrastructure.execution.environments import docker
...
def test_project_containerfile_installs_declared_test_toolchains():
    content = (ROOT / "src/voidcube/infrastructure/execution/containerfiles/podman-agent.Containerfile").read_text(
        encoding="utf-8"
    )
