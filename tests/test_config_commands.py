...
def test_config_command_layer_owns_canonical_cli_handlers():
    command_source = (ROOT / "src" / "voidcube" / "interfaces" / "cli" / "config_commands.py").read_text(
        encoding="utf-8"
    )
    ...
    main_tree = ast.parse(
        (ROOT / "src" / "voidcube" / "interfaces" / "cli" / "entrypoints" / "operations.py").read_text(
            encoding="utf-8"
        )
    )
    imports = {
        ...
        and node.module == "config_commands"
        and node.level == 2
...
def test_set_config_value_writes_terminal_config_and_removes_legacy_env(tmp_path, monkeypatch):
    ...
    from voidcube.interfaces.cli.config_commands import set_config_value
...
def test_config_command_dispatches_path_without_loading_full_config(tmp_path, monkeypatch, capsys):
    ...
    from voidcube.interfaces.cli.config_commands import config_command
