...
from voidcube.systems.supervisor.endogenous_drive_cycle import (
...
from voidcube.systems.supervisor.endogenous_candidate_pipeline import (
...
from voidcube.systems.supervisor.endogenous_lm_evidence import (
    build_lm_evidence_packet_from_runtime_config,
)
from voidcube.systems.supervisor.endogenous_needs import detect_needs
from voidcube.systems.supervisor.endogenous_generation_snapshot import (
    build_lm_task_generation_context_snapshot,
)
from voidcube.systems.supervisor.endogenous_meta_cognition import (
...
from voidcube.systems.supervisor.endogenous_state_projection import project_drive_history
from voidcube.systems.supervisor.endogenous_drive_context import build_drive_context
from voidcube.systems.supervisor.endogenous_cognition_state import build_judgement_core_projection
from voidcube.systems.supervisor.endogenous_strategy_memory import normalize_endogenous_strategy_memory
from voidcube.systems.supervisor.endogenous_self_iteration import (
...
def _body_evaluation_authorization() -> dict:
    return {
        ...
        "candidate_ref": "refs/voidcube/candidates/chain-test",
        "changed_files": ["src/voidcube/runtime/agent/demo.py"],
...
def _endogenous_drive_input_payload(
    *,
    ...
    user_idle: int = 900,
    employee_execution_idle: int = 900,
    memory_idle: int = 900,
    ...
    return {
        ...
        "idle_seconds": {
            "user": user_idle,
            "employee_execution": employee_execution_idle,
            "memory": memory_idle,
        },
        "user_chain_signal": {
...
@pytest.mark.asyncio
@pytest.mark.unit
async def test_api_b_judgement_task_summaries_keep_employee_execution_lane_out_of_api_b_judgement(tmp_path):
    supervisor = _make_supervisor(tmp_path)
    ...
    lane = supervisor._employee_execution_lane_task_summaries(limit=5)
...
@pytest.mark.asyncio
@pytest.mark.unit
async def test_endogenous_drive_api_b_judgement_count_excludes_employee_execution_lane(tmp_path):
    supervisor = _make_supervisor(tmp_path)
    supervisor._touch_gateway_activity = AsyncMock()  # type: ignore[method-assign]
    ...
    assert perception["employee_dispatch_count"] == 0
    assert perception["employee_running_count"] == 1


@pytest.mark.asyncio
@pytest.mark.unit
async def test_endogenous_drive_suppresses_body_growth_while_employee_execution_lane_is_active(tmp_path):
    supervisor = _make_supervisor(tmp_path)
    supervisor._touch_gateway_activity = AsyncMock()  # type: ignore[method-assign]
...
async def test_cognitive_self_regulation_tightens_when_proposal_explanations_are_inconsistent(tmp_path):
    ...
    async def fake_drive_input(_request=None):
        return {
            ...
            "idle_seconds": {"user": 1200, "employee_execution": 1200, "memory": 1200},
            "activity": {"active_sessions": 0, "counts": {"error_count": 0, "uncertainty_high_count": 0}},
            "task_family_decisions": {
                "memory_maintenance": {"eligible_for_planning": True, "eligible_for_execution": True},
                "self_learning": {"eligible_for_planning": True, "eligible_for_execution": True},
...
async def test_prompt_packet_budget_preserves_api_b_judgement_snapshot_under_large_context(tmp_path):
    from voidcube.systems.supervisor.endogenous_drive_prompts import _prompt_facing_evidence_packet
...
def test_cognitive_briefing_renders_task_shape_hint_as_secondary_hint():
    from voidcube.systems.supervisor.endogenous_drive_prompts import _render_cognitive_briefing
...
def test_prompt_packet_trim_stage_order_follows_charter_attention_policy():
    from voidcube.systems.supervisor.endogenous_drive_prompts import _ensure_prompt_packet_budget
...
async def test_endogenous_drive_cognitive_briefing_prefers_context_layers(tmp_path):
    from voidcube.systems.supervisor.endogenous_drive_prompts import build_endogenous_task_generation_payload
...
async def test_completed_learning_structure_mapping_reaches_planned_body_task(tmp_path):
    ...
    async def fake_drive_input(_request=None):
        return {
            ...
            "completed_learning_tasks": [
                {
                    ...
                    "conclusion": "Update src/voidcube/runtime/agent/stream_handler.py only.",
            ...
            "endogenous_drive_policy": {
                ...
                "body_improvement_editable_dirs": ["src/voidcube/runtime/agent/", "src/voidcube/extensions/tools/"],
    ...
    assert body_task["constraints"]["target_paths"] == ["src/voidcube/runtime/agent/stream_handler.py"]
...
async def test_endogenous_drive_still_plans_learning_candidates_with_active_sessions(tmp_path):
    ...
    async def fake_drive_input(request: dict | None = None):
        ...
        return {
            "checks": {},
            "idle_seconds": {"user": 1000, "employee_execution": 1000, "memory": 1000},
            "activity": {
                "counts": {},
                "active_sessions": 2,
                "recent_metadata": {
...
def test_autonomous_chain_store_exposes_api_b_judgement_employee_execution_lane_and_writeback_projections(tmp_path):
    supervisor = _make_supervisor(tmp_path)
    ...
    employee_dispatch_titles = [task.title for task in store.list_employee_dispatch_tasks()]
    employee_running_titles = [task.title for task in store.list_employee_running_tasks()]
    api_a_lane_titles = [task.title for task in store.list_employee_execution_lane_tasks()]
    ...
    assert employee_dispatch_titles == []
    assert employee_running_titles == ["API-A 执行段链路项"]
...
def test_autonomous_chain_store_recovers_projection_from_mem_governance_events(tmp_path):
    ...
    assert [task.task_id for task in store.list_employee_dispatch_tasks()] == ["recover-task-1"]
...
def test_mem_governance_recovery_inherits_context_across_partial_events(tmp_path):
    ...
    assert task.constraints == {"allowed_paths": ["src/voidcube/runtime/agent/"]}
