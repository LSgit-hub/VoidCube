...
from voidcube.infrastructure.config.environment import is_placeholder_secret, load_VoidCube_dotenv
from voidcube.infrastructure.providers.auth import get_auth_status, read_credential_pool, write_credential_pool
...
""",
    ...
    from voidcube.infrastructure.config.configuration import load_config, migrate_config
...
def test_retired_tool_progress_env_migrates_once_on_current_config(
    ...
    import voidcube.infrastructure.config.configuration as config_module
...
def test_retired_tool_progress_env_is_not_configurable():
    from voidcube.infrastructure.config.configuration import OPTIONAL_ENV_VARS
...
def test_tool_config_platforms_ignore_retired_messaging_env(monkeypatch):
    ...
    from voidcube.interfaces.cli.tools_config import _get_enabled_platforms
...
def test_save_env_value_updates_current_process_env(tmp_path, monkeypatch):
    ...
    from voidcube.infrastructure.config.configuration import save_env_value
...
def test_credential_pool_round_trips_by_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "voidcube.infrastructure.providers.credentials.auth_store_path",
        lambda: auth_path,
    )
...
def test_custom_provider_pool_key_uses_providers_map(monkeypatch):
    from voidcube.infrastructure.providers.credential_pool import get_custom_provider_pool_key

    monkeypatch.setattr(
        "voidcube.infrastructure.providers.credential_pool._load_config_safe",
        lambda: {
...
def test_login_api_key_save_failure_does_not_echo_secret(monkeypatch, capsys):
    from voidcube.interfaces.cli.auth import _login_api_key

    monkeypatch.setattr("getpass.getpass", lambda prompt="": "sk-real-secret-token-123456789")
    monkeypatch.setattr(
        "voidcube.infrastructure.config.configuration.save_env_value",
        lambda key, value: (_ for _ in ()).throw(RuntimeError("disk full")),
    )
    monkeypatch.setattr("voidcube.infrastructure.config.configuration.get_env_path", lambda: "C:/Users/test/.VoidCube/.env")
