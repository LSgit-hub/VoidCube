...
from voidcube.systems.supervisor.endogenous_body_mapping import build_body_structure_mapping
from voidcube.systems.supervisor.endogenous_materialization import (
    materialize_lm_proposals_for_deliberation,
)
from voidcube.systems.supervisor.endogenous_pressure import memory_maintenance_urgency
from voidcube.systems.supervisor.supervisor import SupervisorConfig
...
class TestEndogenousDriveWithGatewayData:
    ...
    def test_memory_maintenance_urgency_ignores_generic_agent_idle(self):
        formal = {
            "idle_seconds": {
                "user": 900,
                "employee_execution": 0,
                "agent": 900,
                "memory": 900,
            }
        }
        missing_employee_execution = {
            "idle_seconds": {
                "user": 900,
                "agent": 900,
                "memory": 900,
        ...
        assert memory_maintenance_urgency(missing_employee_execution) == memory_maintenance_urgency(no_agent)
        assert memory_maintenance_urgency(formal) == memory_maintenance_urgency(no_agent)
        assert memory_maintenance_urgency(formal_idle) > memory_maintenance_urgency(missing_employee_execution)

    def test_drive_generates_truthfulness_when_errors_exist(self):
        engine = EndogenousDriveEngine()
        drive_input = {
            "checks": {},
            "idle_seconds": {"user": 1000, "employee_execution": 1000},
...
class TestServiceRuntimeLifecycle:
    ...
    def test_periodic_tasks_start_and_stop(self, tmp_path):
        from voidcube.systems.supervisor.supervisor import Supervisor, SupervisorConfig
...
class TestCLIExecutorCanonicalPath:
    ...
    def test_execution_facade_routes_self_evolution_to_executor(self):
        from voidcube.systems.execution.facade import VoidCubeExecutionFacade
        ...
        async def _run():
            from voidcube.systems.supervisor.autonomous_chain_store import AutonomousChainExecutionRequest
            req = AutonomousChainExecutionRequest(
                task_id="t1", kind="general_self_evolution",
                git_lineage={"source_commit": "def", "candidate_commit": "abc", "rollback_commit": "def", "changed_files": ["src/voidcube/runtime/agent/x.py"]},
...
class TestEndogenousDriveErrorBridge:
    ...
    def test_no_candidates_when_fully_blocked(self):
        ...
        idle = {
            "checks": {},
            "idle_seconds": {"user": 100, "employee_execution": 100, "memory": 100},
    ...
    def test_body_improvement_candidate_respects_recent_backlog_cooldown(self):
        ...
        idle = {
            ...
            "endogenous_drive_policy": {
                ...
                "body_improvement_editable_dirs": ["src/voidcube/runtime/agent/"],
    ...
    def _body_improvement_drive_input(*, conclusion: str) -> dict:
        return {
            ...
            "endogenous_drive_policy": {
                "body_improvement_min_quality": 60.0,
                "body_improvement_cooldown_hours": 24,
                "body_improvement_editable_dirs": ["src/voidcube/runtime/agent/", "src/voidcube/extensions/tools/", "systems/"],
    ...
    def test_learning_evidence_maps_only_to_canonical_body_nodes(self):
        ...
        drive_input = self._body_improvement_drive_input(
            conclusion=(
                "Update src/voidcube/runtime/agent/stream_handler.py after validating the stream display fix. "
                "Do not modify src/voidcube/systems/supervisor/planning_runtime.py or "
                "src/voidcube/runtime/agent/credential_pool.py."
            )
        )
        ...
        assert "src/voidcube/runtime/agent/stream_handler.py" in projection["target_paths"]
        assert all(not path.startswith("systems/") for path in projection["target_paths"])
        assert "src/voidcube/runtime/agent/credential_pool.py" not in projection["target_paths"]
        assert projection["editable_dirs"] == ["src/voidcube/runtime/agent/", "src/voidcube/extensions/tools/"]
    ...
    def test_completed_learning_generates_governed_body_improvement_without_lm(self):
        engine = EndogenousDriveEngine()
        drive_input = self._body_improvement_drive_input(
            conclusion="Improve src/voidcube/runtime/agent/stream_handler.py based on the verified stream result."
        )
        ...
        assert "src/voidcube/runtime/agent/stream_handler.py" in candidate.constraints["target_paths"]
    ...
    def test_lm_body_improvement_is_bound_to_program_structure_mapping(self):
        engine = EndogenousDriveEngine()
        drive_input = self._body_improvement_drive_input(
            conclusion="Improve src/voidcube/runtime/agent/stream_handler.py using the verified stream finding."
        )
