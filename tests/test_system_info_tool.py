...
from voidcube.extensions.tools import ops_register
from voidcube.extensions.tools.model_tools import get_tool_definitions, handle_function_call


@pytest.mark.unit
def test_system_info_reports_effective_podman_workspace(monkeypatch):
    monkeypatch.setattr(
        "voidcube.infrastructure.execution.terminal_tool._get_env_config",
        lambda: {
            "env_type": "podman",
            "cwd": "/workspace",
...
@pytest.mark.unit
def test_system_info_is_available_through_the_cli_tool_registry(monkeypatch):
    monkeypatch.setattr(
        "voidcube.infrastructure.execution.terminal_tool._get_env_config",
        lambda: {
            "env_type": "local",
