...
from voidcube.systems.body_registry import BodyRegistry, BodySlotMeta
from voidcube.systems.governor import GovernorRequest
from voidcube.systems.lifecycle import LifecycleExecutionReport
...
def test_mem_governor_bridge_records_evolution_lineage_summary(tmp_path):
    ...
    assert lineage["changed_files"] == ["src/voidcube/runtime/agent/stream_handler.py"]
    assert lineage["evolution_boundary"]["ok"] is True
    assert lineage["evolution_boundary"]["allowed_files"] == [
...
def test_mem_governor_bridge_records_boundary_violations_in_lineage(tmp_path):
    ...
    slot_meta = BodySlotMeta(
        ...
        changed_files=[
            "src/voidcube/runtime/agent/stream_handler.py",
            "src/voidcube/systems/body_registry.py",
        ],
    )
    request = GovernorRequest(
        request_id="switch-3",
    ...
    assert boundary["allowed_files"] == [
        "src/voidcube/runtime/agent/stream_handler.py"
    ]
    assert boundary["forbidden_files"] == ["src/voidcube/systems/body_registry.py"]
    assert boundary["violations"] == ["src/voidcube/systems/body_registry.py"]
