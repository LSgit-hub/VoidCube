...
from voidcube.systems.execution.adapters import (
...
def _seed_body_repo(tmp_path: Path, *, probe_ready: bool) -> None:
    (tmp_path / "src" / "voidcube" / "interfaces" / "cli").mkdir(parents=True, exist_ok=True)
    (tmp_path / "src" / "voidcube" / "interfaces" / "cli" / "root_launcher.py").write_text(
        "print('slot launch')\n",
        encoding="utf-8",
    )
