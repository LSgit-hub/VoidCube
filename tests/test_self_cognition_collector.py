...
def _build_project(root: Path) -> None:
    ...
    (root / "agent" / "main.py").write_text(
        "import json\nfrom systems.worker import run\n\n"
        "def lazy_import():\n    import pathlib\n",
        encoding="utf-8",
    )
    (root / "systems" / "worker.py").write_text(
        "def run():\n    return None\n",
        encoding="utf-8",
    )
...
def test_collection_is_stable_at_a_fixed_time(tmp_path: Path):
    ...
    agent_main = next(item for item in first.modules if item.module == "agent.main")
    assert agent_main.dependencies == ("json", "systems")
...
def test_missing_roots_and_invalid_python_are_reported_as_gaps(tmp_path: Path):
    ...
    (tmp_path / "agent" / "broken.py").write_text("def broken(:\n", encoding="utf-8")

    snapshot = _collector(tmp_path).collect(collected_at=NOW)

    assert "module_parse_failed:agent.broken" in snapshot.known_gaps
    assert "source_root_missing:systems" in snapshot.known_gaps
    assert "health_failed:configuration" in snapshot.known_gaps
    assert "systems" in snapshot.uncovered_areas
...
def test_collector_has_no_supervisor_or_legacy_store_imports():
    collector_path = Path(__file__).parents[1] / "systems" / "self_cognition" / "collector.py"
