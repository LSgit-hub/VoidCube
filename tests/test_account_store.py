...
from voidcube.systems.supervisor.account_store import (
...
from voidcube.systems.supervisor.ui_runtime import SupervisorUIRuntime
...
class TestSaveAndDelete:
    def test_save_creates_account(self, monkeypatch) -> None:
        ...
        try:
            ...
            monkeypatch.setattr(
                "voidcube.systems.supervisor.account_store._accounts_path",
...
def _mock_verification_client(monkeypatch, handler) -> None:
    ...
    monkeypatch.setattr("voidcube.systems.supervisor.account_store.httpx.AsyncClient", create_client)
...
async def test_inconclusive_verification_preserves_working_account_status(monkeypatch) -> None:
    ...
    saved_accounts = []
    monkeypatch.setattr("voidcube.systems.supervisor.account_store.load_accounts", lambda: [account])
    monkeypatch.setattr(
        "voidcube.systems.supervisor.account_store.verify_account",
        lambda _account: _async_result(
            {"status": "error", "detail": "平台暂时拒绝验证请求 (HTTP 412)"}
        ),
    )
    monkeypatch.setattr(
        "voidcube.systems.supervisor.account_store.save_account", saved_accounts.append
    )
