...
class _SessionRepository:
    ...
    def resolve_resumable_session_id(self, session_id: str) -> str:
        self.events.append(("resolve_resumable", session_id))
        return str(self.sessions.get(session_id, {}).get("resumable_id") or session_id)
...
def test_resume_and_hydrate_follow_latest_compression_continuation() -> None:
    repository = _SessionRepository()
    repository.sessions["root"] = {
        "id": "root",
        "resumable_id": "continuation",
    }
    repository.sessions["continuation"] = {"id": "continuation"}
    repository.messages["continuation"] = [
        {"role": "assistant", "content": "compressed history"}
    ]

    hydration = hydrate_session(repository=repository, session_id="root")
    resumed = resume_session(
        repository=repository,
        current_session_id="current",
        target_session_id="root",
        session_start=datetime(2026, 8, 16),
    )

    assert hydration.session_id == "continuation"
    assert resumed.state.session_id == "continuation"
    assert resumed.state.conversation_history == (
        {"role": "assistant", "content": "compressed history"},
    )
