...
from memai.application.memory_service import MemoryService, SessionCreate, TurnCreate
...
async def test_explicit_json_export_is_consistent_and_scoped(tmp_path):
    ...
    assert "time_summaries" in payload["tables"]
    assert "time_summary_links" in payload["tables"]
    assert "session_summary_sources" in payload["tables"]
