import voidcube.interfaces.cli.turn_agent_route_runtime as route_module
from voidcube.interfaces.cli.turn_agent_route_runtime import (
...
def test_route_runtime_projects_runtime_credentials_and_clears_overrides_without_tier(monkeypatch):
    ...
    def resolve(message, routing, credentials, **_kwargs):
...
def test_route_runtime_adds_fast_mode_overrides(monkeypatch):
    monkeypatch.setattr(
        route_module,
        "resolve_turn_route",
        lambda *_args, **_kwargs: {"model": "fast-model"},
    )
    monkeypatch.setattr(
        route_module,
