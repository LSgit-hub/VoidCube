...
from voidcube.extensions.tools.media.media_tool import (
...
def test_media_url_uses_canonical_supervisor_config(monkeypatch) -> None:
    ...
    with patch("voidcube.infrastructure.config.system.load_config_from_env", return_value=config):
...
def test_media_play_posts_to_supervisor_enqueue(monkeypatch) -> None:
    ...
    with patch("voidcube.extensions.tools.media.media_tool.httpx.post", return_value=response) as post:
        result = media_play(
...
def test_media_display_uploads_local_file_before_delivery(monkeypatch, tmp_path) -> None:
    ...
    with patch("voidcube.extensions.tools.media.media_tool.httpx.post", side_effect=[uploaded, pushed]) as post:
...
def test_media_display_rejects_auto_employee_results_before_upload(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("SUPERVISOR_MEDIA_URL", "http://127.0.0.1:6002")
    source = tmp_path / "internal-report.md"
    source.write_text("internal", encoding="utf-8")

    with patch("voidcube.extensions.tools.media.media_tool.httpx.post") as post:
        result = media_display(
            file_path=str(source),
            main_runtime={
                "autonomous_task": {
                    "task_id": "auto-1",
                    "metadata": {"assist_mode": False},
                }
            },
        )

    assert '"status": "error"' in result
    assert "autonomous_employee_delivery_forbidden" in result
    post.assert_not_called()


def test_media_display_allows_assist_employee_results(monkeypatch) -> None:
    monkeypatch.setenv("SUPERVISOR_MEDIA_URL", "http://127.0.0.1:6002")
    response = Mock()
    response.raise_for_status.return_value = None
    response.json.return_value = {"status": "ok", "current": {"delivery_id": "d1"}}
