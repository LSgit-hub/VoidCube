...
from voidcube.interfaces.cli.ops import dashboard
import voidcube.interfaces.cli.status as cli_status
...
def test_dashboard_builds_supervisor_only_status_summary(monkeypatch):
    ...
    assert built["status"]["read_model_version"] == "13"
...
def test_build_dashboard_reads_supervisor_observation_input_without_extra_guard_shell(monkeypatch):
    monkeypatch.setattr(
        dashboard,
        "fetch_supervisor_state",
        lambda: {
...
def test_build_dashboard_prefers_supervisor_autonomous_observation_board(monkeypatch):
                    ...
                    "segments": [
                        {
                            ...
                            "source_label": "API-B",
                            "stage_label": "判断在途",
                            "items": [{"title": "API-B judgement task", "display_status": "待判断"}],
                        },
                        {
    ...
    assert built["chain"]["api_b_judgement"] == 1
    assert built["chain"]["employee_running"] == 1
    assert built["chain"]["employee_dispatch"] == 2
    ...
    assert built["chain"]["segments"][0]["stage_label"] == "判断在途"
    assert built["chain"]["segments"][0]["source_label"] == "API-B"
    assert "owner" not in built["chain"]["segments"][0]
    assert built["chain"]["segments"][1]["label"] == "API-B 已转交投影"
    assert built["chain"]["segments"][0]["title"] == "API-B judgement task"


def test_build_dashboard_employee_observation_reads_supervisor_task_lane_only(monkeypatch):
    monkeypatch.setattr(
        dashboard,
        "fetch_supervisor_state",
        lambda: {
            "autonomous_observation": {
                "counts": {"employee_running": 1},
                "loop": {
                    "stage_cards": [
                        {
                            "stage_key": "employee_execution",
    ...
    assert built["employee_observation"]["current_scene"] == "learning"
    assert built["employee_observation"]["current_scene_label"] == "自主学习"
    assert built["employee_observation"]["status_label"] == "他处执行中"
    assert built["employee_observation"]["task_id"] == "learn-supervisor-1"
    assert built["employee_observation"]["task_title"] == "Supervisor autonomous task"
    assert built["employee_observation"]["task_kind_label"] == "自主学习"
    assert built["employee_observation"]["subagent_foreground_count"] == 2
    assert built["employee_observation"]["subagent_background_count"] == 1
    assert built["employee_observation"]["subagent_focus_tool"] == "read_file"
    assert built["employee_observation"]["subagent_focus_preview"] == ""
    assert built["employee_observation"]["session_id"] == "cli-session-supervisor"
    assert built["employee_observation"]["lease_status"] == "observed"
    assert "cli-session-user" not in built["employee_observation"]["presence_summary"]
...
def test_build_dashboard_prefers_supervisor_recent_activity_projection(monkeypatch):
            ...
            "autonomous_observation": {
                "board": {
                    "recent_activity": {
                        ...
                        "summary": "API-B 已更新 替身改进 的判断，并决定是否转交 API-A。",
                        "source_label": "API-B",
    ...
    assert built["recent_activity"]["phase_label"] == "判断转交"
    assert built["recent_activity"]["title"] == "Supervisor projected summary"
    assert built["recent_activity"]["summary"] == "API-B 已更新 替身改进 的判断，并决定是否转交 API-A。"
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            ...
            "employee_observation": {
                ...
                "presence_summary": "执行面: 会话 supervisor 正在进行 员工代理执行（静默 3s）",
                "chain_reason": "链路: 该链路项已被其他 员工代理执行面认领",
                "activity_text": "执行流: 链路项正在其他 员工代理执行面中运行",
            },
            "observation_input": {
