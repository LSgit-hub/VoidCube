...
import voidcube.interfaces.cli.application as cli
from voidcube.interfaces.cli.commands.catalog import COMMANDS_BY_CATEGORY, resolve_command
from voidcube.interfaces.cli.commands.execution import initialize_command_execution
from voidcube.interfaces.cli.commands.handlers.display import (
    SessionStatusDisplayPorts,
    handle_session_status_command,
)
from voidcube.interfaces.cli.commands.handlers.tasks import (
...
from voidcube.application.scheduling.background_task_runtime import BackgroundTaskState
...
def test_tasks_command_help_entry_hides_manual_debug_usage():
    ...
    assert "显示当前子代理状态" in description
...
def _tasks_ports(
    *,
    ...
    render_subagent_task=lambda _ref: None,
    render_subagent_task_log=lambda _ref: None,
...
def test_tasks_handler_renders_one_subagent_detail_by_index():
    ports, output, _invalidated = _tasks_ports(
        has_managers=lambda: True,
        render_subagent_task=lambda ref: "Task detail" if ref == "1" else None,
    )

    handle_tasks_command(parse_cli_command("/tasks 1"), ports=ports)

    assert output == ["Task detail"]


def test_tasks_handler_reports_unknown_detail_reference():
    ports, output, _invalidated = _tasks_ports(has_managers=lambda: True)

    handle_tasks_command(parse_cli_command("/tasks missing"), ports=ports)

    assert output == ["  未知的子代理任务：missing"]


def test_tasks_handler_renders_full_log_by_index():
    ports, output, _invalidated = _tasks_ports(
        has_managers=lambda: True,
        render_subagent_task_log=lambda ref: "Full log" if ref == "1" else None,
    )

    handle_tasks_command(parse_cli_command("/tasks 1 log"), ports=ports)

    assert output == ["Full log"]


def test_tasks_handler_accepts_log_first_alias():
    ports, output, _invalidated = _tasks_ports(
        has_managers=lambda: True,
        render_subagent_task_log=lambda ref: "Full log" if ref == "1" else None,
    )

    handle_tasks_command(parse_cli_command("/tasks log 1"), ports=ports)
...
def test_get_subagent_observability_snapshot_summarizes_active_tasks():
    ...
    assert snapshot["focus_preview"] == "Inspect docs"
