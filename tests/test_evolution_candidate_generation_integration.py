...
from voidcube.systems.evolution_authoring import AIAgentAuthoringAdapter
from voidcube.systems.evolution_evaluation import MetricTarget
from voidcube.systems.supervisor.evolution_candidate_generation_service import (
    EvolutionCandidateGenerationService,
)
from voidcube.systems.supervisor.evolution_candidate_generation_scheduler import (
...
from voidcube.systems.supervisor.endogenous_candidate_factories import (
...
pytestmark = [
    ...
    pytest.mark.skipif(
        shutil.which("tirith") is None,
        reason="native candidate generation requires the Tirith security scanner",
    ),
]
...
def _repository(tmp_path: Path) -> tuple[Path, str]:
    ...
    repository.mkdir()
    agent_package = repository / "src" / "voidcube" / "runtime" / "agent"
    agent_package.mkdir(parents=True)
    (agent_package / "demo.py").write_text(
        "VALUE = 'baseline'\n",
        encoding="utf-8",
    )
    probe_package = (
        repository / "src" / "voidcube" / "systems" / "evolution_evaluation"
    )
    probe_package.mkdir(parents=True)
    (repository / "src" / "voidcube" / "__init__.py").write_text(
        "", encoding="utf-8"
    )
    (repository / "src" / "voidcube" / "systems" / "__init__.py").write_text(
        "", encoding="utf-8"
    )
...
async def test_real_scheduler_shadow_then_manual_native_first_candidate_cycle(
    ...
    request = EvolutionCandidateGenerationRequest.create(
        ...
        source_learning_refs=(
            CandidateLearningReference(
                ...
                target_paths=("src/voidcube/runtime/agent/demo.py",),
            ),
        ),
    ...
    assert _git(
        "show",
        f"{candidate}:src/voidcube/runtime/agent/demo.py",
        cwd=repository,
    ) == "VALUE = 'candidate'"
            ...
            "completed_learning_tasks": [
                {
                    ...
                    "conclusion": "Update src/voidcube/runtime/agent/demo.py using the evaluated candidate.",
