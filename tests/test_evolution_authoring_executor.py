...
from voidcube.infrastructure.execution.task_execution import TaskExecutionBlocked
...
def _repository(tmp_path: Path) -> tuple[Path, str]:
    ...
    demo = repository / "src" / "voidcube" / "runtime" / "agent" / "demo.py"
    demo.parent.mkdir(parents=True)
    demo.write_text("VALUE = 'baseline'\n", encoding="utf-8")
    _git("add", "src/voidcube/runtime/agent/demo.py", cwd=repository)
...
async def test_authoring_executor_creates_tested_candidate_ref_and_cleans_worktree(
    ...
    class Agent:
        async def author(self, context):
            ...
            assert context.allowed_paths == ("src/voidcube/runtime/agent/demo.py",)
            assert harness.worktree is not None
            (harness.worktree / "src/voidcube/runtime/agent/demo.py").write_text(
                "VALUE = 'candidate'\n", encoding="utf-8"
            )
    ...
    assert (
        _git("show", f"{result.candidate_ref}:src/voidcube/runtime/agent/demo.py", cwd=repository).stdout
        == "VALUE = 'candidate'\n"
    )
    assert _git("rev-parse", "HEAD", cwd=repository).stdout.strip() == baseline
    assert (repository / "src/voidcube/runtime/agent/demo.py").read_text(
        encoding="utf-8"
    ) == "VALUE = 'baseline'\n"
...
async def test_authoring_executor_rejects_agent_created_commit(tmp_path: Path):
    ...
    class Agent:
        def author(self, context):
            ...
            _git("add", "src/voidcube/runtime/agent/demo.py", cwd=harness.worktree)
