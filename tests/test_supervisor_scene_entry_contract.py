...
def test_scene_entries_have_explicit_domain_boundaries():
    assert 'data-scene-entry="memory-structure"' in HTML
...
def test_scene_entry_labels_are_accessible_and_point_to_expected_drawers():
    assert 'data-scene-entry="memory-structure" aria-label="Mem 结构与统计"' in HTML
    ...
    assert "target === 'memory-structure'" in HTML
...
def test_autonomous_task_drawer_explicitly_excludes_schedules():
    ...
    assert "board.planning_tasks" in drawer_source
