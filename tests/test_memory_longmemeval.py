...
def test_semantic_config_from_overrides_builds_external_provider():
    ...
    # No overrides use a deterministic local embedder instead of depending on
    # host configuration registration in the test process.
    local = _semantic_config_from_overrides()
    assert local is not None
    assert local.enabled is True
    assert local.provider == "local"
    assert local.model == "char-ngram-v1"
