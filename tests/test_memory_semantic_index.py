...
def test_semantic_index_removes_turn_embedding_when_turn_is_compressed(tmp_path):
    service = _service(tmp_path)
    _insert_turn(service, turn_id="compressed-turn", text="database migration")
    index = _index(service)
    assert index.index_pending() == 1
    ...
    try:
        ...
        remaining = conn.execute(
            ...
            "WHERE source_type = 'turn' AND memory_id = ?",
            ("compressed-turn",),
        ).fetchone()[0]
    finally:
        conn.close()
