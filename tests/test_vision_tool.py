...
def test_vision_tool_uses_shared_auxiliary_router(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "tools.vision_tools._vision_backends_available",
        lambda: True,
    )
    ...
    result = json.loads(
        __import__("tools.vision_tools", fromlist=["vision_analyze_tool"]).vision_analyze_tool(
...
def test_vision_tool_reports_missing_backend(monkeypatch, tmp_path):
    image = tmp_path / "sample.png"
    image.write_bytes(b"\x89PNG\r\n\x1a\n")
    monkeypatch.setattr("tools.vision_tools._vision_backends_available", lambda: False)
