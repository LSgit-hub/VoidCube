...
from voidcube.interfaces.cli.configuration import (
...
from voidcube.infrastructure.config.configuration import (
...
def test_has_configured_api_key_rejects_template_placeholder(monkeypatch):
    monkeypatch.setattr("voidcube.infrastructure.config.configuration.get_env_value", lambda key: "sk-your-key-here")

    assert has_configured_api_key("DEEPSEEK_API_KEY") is False


def test_has_configured_api_key_accepts_real_key(monkeypatch):
    monkeypatch.setattr("voidcube.infrastructure.config.configuration.get_env_value", lambda key: "sk-real-deepseek-token-123456789")
...
def test_api_b_key_configured_uses_provider_default_key_env(monkeypatch):
    ...
    monkeypatch.setattr("voidcube.infrastructure.config.configuration.get_env_value", fake_get_env_value)
...
def test_provider_has_usable_credential_uses_matching_pool(monkeypatch):
    ...
    monkeypatch.setattr("voidcube.infrastructure.config.configuration.get_env_value", lambda key: "")
    monkeypatch.setattr(
...
def test_persist_ollama_provider_uses_shared_pool_and_no_auth():
    updated = persist_ollama_provider(
        {"runtime": {}, "providers": {}},
        model_catalog=["qwen3:8b", "llama3.2"],
        selected_model="llama3.2",
    )

    entry = updated["providers"]["ollama"]
    assert entry["type"] == "ollama"
    assert entry["auth_mode"] == "none"
    assert entry["base_url"] == "http://localhost:11434/v1"
    assert entry["selected_model"] == "llama3.2"
    assert entry["model_catalog"]["models"] == ["qwen3:8b", "llama3.2"]
...
def test_refresh_provider_pool_catalog_uses_existing_env_key(monkeypatch):
    ...
    monkeypatch.setattr("voidcube.interfaces.cli.configuration.get_provider_models_from_api", fake_models)
