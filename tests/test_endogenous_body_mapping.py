...
from voidcube.systems.supervisor.endogenous_body_mapping import (
...
def test_canonical_roots_and_path_boundary_reject_forbidden_mother_system_nodes():
    ...
    assert roots == ["src/voidcube/runtime/agent/", "src/voidcube/extensions/tools/"]
    assert path_within_body_editable_roots(
        "src/voidcube/runtime/agent/stream_handler.py", roots, ["**/credential*"]
    ) is True
    assert path_within_body_editable_roots(
        "src/voidcube/systems/supervisor/planning_runtime.py", roots, []
    ) is False
    assert path_within_body_editable_roots(
        "src/voidcube/runtime/agent/credential_pool.py", roots, ["**/credential*"]
    ) is False
...
def test_mapping_projects_explicit_and_keyword_learning_evidence():
    ...
    projection = build_body_structure_mapping(
        completed_learning_tasks=[
            {
                ...
                "conclusion": "Improve src/voidcube/runtime/agent/stream_handler.py after validation.",
        ...
        policy=_policy(body_improvement_editable_dirs=["src/voidcube/runtime/agent/", "src/voidcube/extensions/tools/"]),
    ...
    assert projection["target_paths"] == [
        ...
        "src/voidcube/extensions/tools/files/file_tools.py",
        "src/voidcube/extensions/tools/files/file_operations.py",
    ]
...
def test_mapping_rejects_learning_without_safe_structure_targets():
    ...
    assert projection == {
        ...
        "learning_quality_score": 90.0,
        "editable_roots": ["src/voidcube/runtime/agent/", "src/voidcube/extensions/tools/"],
    }
