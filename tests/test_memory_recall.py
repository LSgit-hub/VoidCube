...
def test_recall_context_redaction_follows_explicit_memory_switch():
    ...
    assert secret in format_recall_context(results)
    assert secret not in format_recall_context(results, redact_sensitive=True)


def test_recall_context_hides_internal_metadata_by_default() -> None:
    result = {
        "id": "memory-internal-id",
        "tier": "tier2",
        "memory_type": "event",
        "title": "身份事实",
        "summary": "星子是 VoidCube 的持续身份。",
        "score": 0.923,
        "matched_terms": ["身份"],
        "evidence_refs": ["turn-1"],
    }

    context = format_recall_context([result])
    audit_context = format_recall_context(
        [result], include_internal_metadata=True
    )

    assert "身份事实" in context
    assert "星子是 VoidCube 的持续身份" in context
    assert "tier2" not in context
    assert "event" not in context
    assert "memory-internal-id" not in context
    assert "score=" not in context
    assert "evidence=" not in context
    assert "memory-internal-id" in audit_context
...
def test_recall_plan_removes_acknowledgement_and_scope_control_noise():
    plan = build_recall_plan("好的请仅需进行记忆系统的问题检查和缺陷改进")

    assert "好的" not in plan.terms
    assert "检查" not in plan.terms
    assert "记忆系统" in plan.terms
    assert "缺陷改进" in plan.terms
    assert {"报错", "阻塞", "blocker", "error"} <= set(plan.concept_terms)
...
@pytest.mark.asyncio
async def test_search_compressed_excludes_hidden_memories_unless_explicitly_requested(tmp_path):
    service = _service(tmp_path)
    remembered = await service.remember(
        DurableMemoryCreate(
            title="Hidden search candidate",
            summary="This should stay out of ordinary compressed searches.",
        )
    )
    memory_id = remembered["memory"]["memory_id"]

    await service.hide_memory(memory_id)
    ...
    ordinary = await service.search_compressed({"query": "hidden search candidate"})
    assert memory_id not in {item["memory_id"] for item in ordinary["results"]}

    explicit = await service.search_compressed(
        {"query": "hidden search candidate", "include_hidden": True}
    )
    assert memory_id in {item["memory_id"] for item in explicit["results"]}


@pytest.mark.asyncio
async def test_idempotent_retry_does_not_resurrect_superseded_memory(tmp_path):
    service = _service(tmp_path)
    old_request = DurableMemoryCreate(
        title="Retry lifecycle decision",
        summary="The old decision is no longer current.",
    )
    old = await service.remember(old_request)
    old_id = old["memory"]["memory_id"]
    replacement = await service.remember(
        DurableMemoryCreate(
            title="Retry lifecycle decision updated",
            summary="The replacement decision is current.",
            supersedes_memory_ids=[old_id],
        )
    )

    await service.remember(old_request)
    ...
    try:
        ...
        conn.close()
    assert status == ("superseded", replacement["memory"]["memory_id"])


@pytest.mark.asyncio
async def test_search_compressed_bounds_limit_and_rejects_non_integer(tmp_path):
    service = _service(tmp_path)
    bounded = await service.search_compressed({"limit": -10})
    assert len(bounded["results"]) <= 1

    with pytest.raises(HTTPException, match="limit must be an integer"):
        await service.search_compressed({"limit": "many"})
...
def test_recall_deduplicates_text_that_mostly_contains_another_result(tmp_path):
    ...
    conn = open_memory_sqlite(service._db_path)
    try:
        from memai.application.recall import recall_memories
...
async def test_recall_mixes_recent_tier1_and_durable_tier2(tmp_path):
    ...
    assert "数据库迁移决策" in result["context"]
    assert "tier1" not in result["context"]
    assert "tier2" not in result["context"]
    assert "id=event-migration" not in result["context"]
...
@pytest.mark.asyncio
async def test_tier2_recall_excludes_active_memory_sourced_from_evaluation_turn(tmp_path):
    service = _service(tmp_path)
    now = datetime.now(timezone.utc)
    _insert_turn(
        service,
        turn_id="evaluation-source",
        session_id="evaluation-session",
        text="QWERT-9901 evaluation source",
        timestamp=now,
        tags=["Evaluation"],
    )
    _insert_compressed(
        service,
        memory_id="normal-tier2",
        title="QWERT-9901 normal",
        summary="Normal durable memory about QWERT-9901.",
        timestamp=now,
    )
    _insert_compressed(
        service,
        memory_id="polluted-tier2",
        title="QWERT-9901 polluted",
        summary="Evaluation-derived durable memory about QWERT-9901.",
        timestamp=now,
    )
    conn = open_memory_sqlite(service._db_path)
    try:
        conn.execute(
            ...
            ('["evaluation-source"]', "polluted-tier2"),
        )
        conn.commit()
    finally:
        conn.close()
    ...
    assert [item["id"] for item in result["results"]] == ["normal-tier2"]
...
async def test_turn_pair_write_is_atomic_scoped_and_idempotent(tmp_path):
    ...
    try:
        rows = conn.execute(
            "SELECT speaker, owner_id, workspace_id, tags FROM turns "
    ...
    assert rows == [
        ("agent", "owner-a", "workspace-a", '["evaluation", "suite=memory"]'),
        ("user", "owner-a", "workspace-a", '["evaluation", "suite=memory"]'),
    ]
...
async def test_profile_memory_is_persisted_recalled_and_superseded(tmp_path):
    ...
    try:
        upsert_profile_memory(
            conn,
            profile("profile-1", "docker", "用户偏好 Docker。"),
            owner_id="owner-a",
            workspace_id="workspace-a",
            now=now.isoformat(),
        )
        upsert_profile_memory(
            conn,
            profile("profile-2", "podman", "用户改为偏好 Podman。"),
            owner_id="owner-a",
            workspace_id="workspace-a",
            now=now.isoformat(),
        )
...
async def test_forget_session_deletes_transitive_memory_and_graph_derivatives(tmp_path):
    service = _service(tmp_path)
    now = datetime.now(timezone.utc)
    session_id = "forget-transitive-session"
    turn_id = "forget-transitive-turn"
    marker = "ZETA-PRIVATE-ERASURE-MARKER"
    _insert_turn(
        service,
        turn_id=turn_id,
        session_id=session_id,
    ...
    rows = (
        ("forget-event", "event", [turn_id], [], None, None, ["Private Entity"]),
        ("forget-scene", "scene", [turn_id], [], None, None, []),
        ("forget-arc", "arc", ["forget-scene"], [], None, None, []),
        ("forget-epoch", "epoch", ["forget-arc"], [], None, None, []),
        (
            "forget-identity",
            "event",
            [f"turn:{turn_id}"],
    ...
    try:
        stamp = now.isoformat()
        for memory_id, memory_type, source_turns, evidence_refs, layer, origin, entities in rows:
            conn.execute(
                "INSERT INTO compressed_memories "
                "(memory_id, memory_type, title, summary, timespan_start, timespan_end, "
                "importance, confidence, topics, entities, source_turns, compressed_at, "
                "status, weight, event_kind, identity_layer, evidence_refs, origin_id, "
                "owner_id, workspace_id, memory_domain, created_at) "
                "VALUES (?, ?, ?, ?, ?, ?, 0.8, 0.9, '[]', ?, ?, ?, "
                "'active', 0.8, 'decision', ?, ?, ?, 'local-user', 'default', "
                "'agent_interaction', ?)",
                (
                    memory_id,
                    memory_type,
    ...
    forgotten = await service.forget_memory(
        ForgetRequest(
            session_id=session_id,
            reason="Delete the complete derived memory chain",
            confirmation="FORGET",
        )
    )
    recalled = await service.recall(
        RecallRequest(query=marker, include_tier1=False, min_score=0.0, limit=20)
    )
    conn = open_memory_sqlite(service._db_path)
    try:
        remaining = conn.execute(
            "SELECT memory_id FROM compressed_memories "
            "WHERE memory_id LIKE 'forget-%'"
        ).fetchall()
        graph_rows = conn.execute(
            "SELECT entity_id FROM entity_nodes WHERE entity_id = 'private entity'"
        ).fetchall()
    finally:
        conn.close()

    assert forgotten["deleted_counts"]["compressed_memories"] == len(rows)
    assert remaining == []
    assert graph_rows == []
    assert all("forget-" not in str(item["id"]) for item in recalled["results"])


@pytest.mark.asyncio
async def test_memory_service_redacts_all_durable_write_paths_when_enabled(tmp_path):
    service = _service(tmp_path)
    service.config.redact_before_store = True
    secret = "sk-1234567890abcdefghijklmnop"
    ...
    await service.remember(
        DurableMemoryCreate(
            ...
            topics=[secret],
            evidence_refs=[f"credential:{secret}"],
        )
    )
    await service.recall(
        RecallRequest(
    ...
    try:
        stored_values = [
            ...
            *conn.execute(
                "SELECT title, summary, topics, evidence_refs FROM compressed_memories "
...
@pytest.mark.asyncio
async def test_memory_service_preserves_raw_memory_when_redaction_is_disabled(tmp_path):
    service = _service(tmp_path)
    secret = "sk-1234567890abcdefghijklmnop"
    await service.create_session(SessionCreate(session_id="raw-memory-session"))
    await service.add_turn(
        "raw-memory-session",
        TurnCreate(speaker="user", text=f"credential={secret}"),
    )
    recalled = await service.recall(
        RecallRequest(
            query="credential",
            current_session_id="raw-memory-session",
            min_score=0.0,
        )
    )
    ...
    try:
        stored_text = conn.execute(
            "SELECT text FROM turns WHERE session_id = 'raw-memory-session'"
        ).fetchone()[0]
    finally:
        conn.close()

    assert stored_text == f"credential={secret}"
    assert secret in recalled["context"]
...
async def test_all_direct_memory_reads_and_mutations_enforce_scope(tmp_path):
    ...
    with pytest.raises(HTTPException, match="Turn not found"):
        await service.author_identity_experience(
            SelfIdentityExperienceCreate(
                turn_id="direct-owner-b",
                title="Cross-scope attempt",
                summary="This must not be accepted.",
                self_claim="I claim this experience as my own history.",
                what_changed="My self-understanding changed.",
                continuity_impact="It connects my past and present.",
                agency="chosen",
