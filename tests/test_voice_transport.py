...
from voidcube.systems.voice.audio import AudioPlayer, AudioRecorder
from voidcube.systems.voice.config import VoiceConfig
from voidcube.systems.voice.fingerprint import FingerprintStore, SPEAKER_ENGINE
from voidcube.systems.voice.session import VoiceSessionManager, _extract_wake_query
from voidcube.systems.voice.stt import SpeechToText
from voidcube.systems.voice.vad import SpeechEndpointDetector
from voidcube.systems.voice.wake import (
    KWS_DECODER,
    KWS_ENCODER,
    KWS_JOINER,
    KWS_MODEL_NAME,
...
def test_voice_config_reads_canonical_local_stt_settings(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "voidcube.infrastructure.config.runtime_paths.get_config_path",
        lambda: config_path,
    )
...
async def test_audio_player_prefers_in_process_sounddevice_backend(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("voidcube.systems.voice.audio.shutil.which", lambda name: None)
...
@pytest.mark.asyncio
@pytest.mark.unit
async def test_voice_session_plays_native_companion_audio_without_stt_or_tts(tmp_path):
    native_audio = base64.b64encode(b"native-audio").decode("ascii")
    seen: dict[str, object] = {}

    async def native_companion(*, text: str, audio_path, session_id: str):
        seen["text"] = text
        seen["audio"] = Path(audio_path).read_bytes()
        assert session_id == "voice-native"
        return {
            "status": "ok",
            "reply_text": "原生语音回复",
            "native_audio": {"data": native_audio, "format": "wav"},
        }
    ...
    manager = VoiceSessionManager(
        ...
        companion_callback=native_companion,
    )
    _install_single_utterance_stream(manager)
    manager.stt.transcribe = AsyncMock(side_effect=AssertionError("STT must not run"))  # type: ignore[method-assign]
    manager.tts.synthesize = AsyncMock(side_effect=AssertionError("TTS must not run"))  # type: ignore[method-assign]
    played: list[bytes] = []

    async def play(path, *, stop_event):
        del stop_event
        played.append(Path(path).read_bytes())

    manager.player.play = play  # type: ignore[method-assign]
    manager._temporary_audio_path = (  # type: ignore[method-assign]
        lambda prefix, suffix=".wav": tmp_path / f"{prefix}{suffix}"
    )

    result = await manager.run_once(session_id="voice-native")

    assert result["status"] == "complete"
    assert result["native_audio"] is True
    assert seen["text"] == ""
    assert isinstance(seen["audio"], bytes) and seen["audio"]
    assert played == [b"native-audio"]
