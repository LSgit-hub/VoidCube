...
def test_cli_has_no_retired_config_compatibility():
    ...
    cli_source = (root / "src" / "voidcube" / "interfaces" / "cli" / "application.py").read_text(encoding="utf-8")
...
def test_cli_only_product_has_no_retired_messaging_entrypoints():
    root = Path(__file__).resolve().parents[1]
    main_source = (root / "src" / "voidcube" / "interfaces" / "cli" / "main.py").read_text(encoding="utf-8")
    config_commands_source = (
        root / "src" / "voidcube" / "interfaces" / "cli" / "config_commands.py"
    ).read_text(encoding="utf-8")
    status_source = (root / "src" / "voidcube" / "interfaces" / "cli" / "status.py").read_text(encoding="utf-8")
...
def test_cli_uses_one_builtin_style_without_skin_switching() -> None:
    ...
    from voidcube.interfaces.cli.commands.catalog import resolve_command

    root = Path(__file__).resolve().parents[1]

    assert not (root / "voidcube.interfaces.cli" / "skin_engine.py").exists()
