...
def test_shell_body_profile_projects_worktree_and_origin_manifest(tmp_path):
    ...
    for name in ("agent", "systems", "tests"):
        (worktree / name).mkdir()
    (worktree / "run_agent.py").write_text("", encoding="utf-8")
    (worktree / "config.yaml").write_text("", encoding="utf-8")
    ...
    assert result["present_roots"] == ["agent", "systems", "tests"]
    assert result["has_run_agent"] is True
