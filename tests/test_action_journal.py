...
from voidcube.extensions.tools.registry import ToolRegistry, registry
...
def test_journal_prepare_failure_blocks_side_effect(monkeypatch):
    ...
    monkeypatch.setattr(
        "voidcube.infrastructure.persistence.action_journal.get_action_journal",
...
def test_dynamic_memory_write_uses_journal_and_stable_operation(monkeypatch, tmp_path):
    journal = ActionJournal(tmp_path / "actions.db")
    invoked = []
    monkeypatch.setattr("voidcube.infrastructure.persistence.action_journal.get_action_journal", lambda: journal)
...
def test_stale_prior_dispatch_is_recovered_via_trusted_lease_validator(
    ...
    from voidcube.systems.supervisor.autonomous_chain_store import StaleExecutionLeaseError
