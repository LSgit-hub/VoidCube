...
def test_repeated_terminal_writeback_for_same_lease_is_idempotent(tmp_path):
    store = AutonomousChainStore(tmp_path / "chain.json")
    task = _approved_task(store)
    claimed = store.claim_execution(task.task_id, owner_session_id="owner")
    generation = claimed.execution_lease.generation
    ...
    first = store.finalize_execution(
        task.task_id,
    ...
    first_decision_count = len(first.decision_history)
    repeated = store.finalize_execution(
        task.task_id,
        generation=generation,
        attempt_id=attempt_id,
        status="completed",
        actor="cli_agent",
        reason="response was lost, retrying",
    )

    assert repeated.status == "completed"
    assert repeated.decision_reason == "finished"
    assert len(repeated.decision_history) == first_decision_count
